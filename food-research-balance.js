...
`;
  ...
  window.FOOD_RESEARCH_APPLY_BALANCE_PATCH = function (body) {
    ...
    out = out
      ...
      .replaceAll('bullets.forEach(b=>b.draw());eBullets.forEach(b=>b.draw());', "bullets.forEach(b=>b.draw());eBullets.forEach(function(b){if(!b||typeof b.draw!=='function')return;const dawn=currentBgIdx===8,oldColor=b.color;if(dawn)b.color='#00e5ff';b.draw();if(FR_MOBILE_PERF){b.color=oldColor;return;}ctx.save();ctx.strokeStyle='#020617';ctx.lineWidth=3;ctx.globalAlpha=.96;ctx.beginPath();ctx.arc(b.x,b.y,(b.r||5)+2,0,Math.PI*2);ctx.stroke();if(dawn){ctx.fillStyle='#ecfeff';ctx.beginPath();ctx.arc(b.x,b.y,Math.max(2,(b.r||5)*.38),0,Math.PI*2);ctx.fill();}ctx.restore();b.color=oldColor;});")
