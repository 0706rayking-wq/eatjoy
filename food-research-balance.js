...
`;
  ...
  window.FOOD_RESEARCH_APPLY_BALANCE_PATCH = function (body) {
    ...
    out = out
      ...
      .replace('bullets.forEach(b=>b.draw());eBullets.forEach(b=>b.draw());', "bullets.forEach(b=>b.draw());eBullets.forEach(function(b){if(!b||typeof b.draw!=='function')return;if(currentBgIdx===8){const oldColor=b.color;b.color='#00e5ff';b.draw();ctx.save();ctx.strokeStyle='#082f49';ctx.lineWidth=2.5;ctx.globalAlpha=.95;ctx.beginPath();ctx.arc(b.x,b.y,(b.r||5)+2,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#ecfeff';ctx.beginPath();ctx.arc(b.x,b.y,Math.max(2,(b.r||5)*.38),0,Math.PI*2);ctx.fill();ctx.restore();b.color=oldColor;}else b.draw();});")
