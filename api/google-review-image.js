...
module.exports = async function handler(request, response) {
  ...
  try {
    const reviewKey = String(request.query.reviewKey || request.query.reviewerId || '').trim();
    ...
    if (!reviewKey || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      return response.status(400).send('Invalid screenshot request');
    }
    if (!secret || !verifyReviewSignature(reviewKey, date, signature, secret)) {
      return response.status(403).send('Forbidden');
    }

    const image = await screenshotReview(parseReviewKey(reviewKey));
