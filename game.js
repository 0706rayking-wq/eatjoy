...
var GAME_JS_BODY = [
  ...
  '  if(d<sRange && angle<=0 && angle>=-Math.PI){',
  '   e.takeDamage(currentForm.bulletDmg*2.2);burst(e.x,e.y,currentForm.bulletColor,5);',
  '  }',
  ' });',
  ' if(boss&&!boss._defeated){',
  '  const dx=boss.x-player.x, dy=boss.y-player.y;',
  '  const d=Math.hypot(dx,dy), angle=Math.atan2(dy,dx);',
  '  if(d<sRange+20 && angle<=0 && angle>=-Math.PI) boss.takeDamage(currentForm.bulletDmg*2.2);',
  ' }',
  '',
  ' let reflected=0;',
  ...
  '  const d=Math.hypot(dx,dy), angle=Math.atan2(dy,dx);',
  '  if(reflected<4 && d<sRange+24 && angle<=0 && angle>=-Math.PI){',
