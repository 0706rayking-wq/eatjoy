(function () {
  const specs = {
    ...
    truffle_thunder:{name:'松露雷將',passive:'雷鏈麻痺：攻擊有 10% 機率連鎖，並短暫麻痺命中敵人',skill1:'雷環護身',skill1Cost:30,skill1Cooldown:13,skill1Desc:'持續 10 秒環繞半徑 80 的電流；每 0.5 秒造成 10 傷害並彈開敵人，首領不擊退但減速 15%。',skill2:'雷域推進',skill2Cost:70,skill2Cooldown:23,skill2Desc:'生成直徑 120、向上緩慢推進的雷電立場 6 秒；每 0.5 秒造成 20 傷害。'},
    dragonfruit_emperor:{passive:'灼燒光環：185 範圍持續灼燒，攻擊灼燒敵人 +20%',skill1:'雙翼龍焰',skill1Cost:38,skill1Cooldown:14,skill1Desc:'向上方左右各噴出寬 70 火焰 4.5 秒；每 0.5 秒造成 30 傷害並附加灼燒，同一目標每 0.5 秒最多受傷一次。',skill2:'龍隕雙星',skill2Cost:75,skill2Cooldown:25,skill2Desc:'間隔 2 秒砸下 2 顆直徑約畫面 1/3 的龍隕石；預警 0.7 秒，中心造成 90、外圍造成 100 傷害並灼燒；一般敵人暈眩 1.5 秒，首領暈眩 1 秒。'},
...
;(function(){
 ...
 function frV2UpdateFields(now){
  for(let i=frV2.fields.length-1;i>=0;i--){const f=frV2.fields[i];
   ...
   else if(f.kind==='truffleRing'&&now>=f.nextTick){f.nextTick=now+500;frV2Area(player.x,player.y,80,10,'skill',function(t){frV2Burst(t.x,t.y,'#fde047',8);frRingFx(t.x,t.y,'#ffffff',32,240,3);frBoltFx(player.x,player.y,t.x,t.y,'#fde047',240,0,4);if(frV2Boss(t))frV2Slow(t,650,.85);else frV2Push(t,player.x,player.y,100);});}
   else if(f.kind==='truffleDomain'){f.y-=.85;if(now>=f.nextTick){f.nextTick=now+500;frV2Targets().forEach(function(t){if(frV2Distance(t,f)<=60+(t.r||18))frV2SkillDamage(t,20,'skill');});}}
   else if(f.kind==='dragonBreath'&&now>=f.activate&&now>=f.nextTick){f.nextTick=now+500;const spread=.22,length=CH,x1=player.x-18,y1=player.y-18,x2=player.x+18,y2=player.y-18,tx1=x1-Math.sin(spread)*length,ty1=y1-Math.cos(spread)*length,tx2=x2+Math.sin(spread)*length,ty2=y2-Math.cos(spread)*length;frV2Targets().forEach(function(t){if(frV2DistanceToSegment(t,x1,y1,tx1,ty1)<=35+(t.r||18)||frV2DistanceToSegment(t,x2,y2,tx2,ty2)<=35+(t.r||18)){frV2SkillDamage(t,30,'skill');frV2Burn(t,2500,16);}});}
   else if(f.kind==='popcornDrop'&&now>=f.activate&&!f.exploded){f.exploded=true;frV2ExplodePopcorn(f.x,f.y);}
   else if(f.kind==='meteor'&&now>=f.activate&&!f.exploded){f.exploded=true;f.explodedAt=now;frV2Targets().forEach(function(t){const d=frV2Distance(t,f);if(d<=f.r+(t.r||18)){frV2SkillDamage(t,d<=f.r*.45?90:100,'skill');frV2Burn(t,3500,16);t._frV2ParalyzedUntil=Math.max(t._frV2ParalyzedUntil||0,now+(frV2Boss(t)?1000:1500));}});frV2Burst(f.x,f.y,'#fb923c',35);frShakeFx(12,520);frFlashFx('#f97316',320,.26);}
 ...
 function frV2Draw(){
  const now=frV2Now();ctx.save();ctx.lineWidth=3;frV2.fields.forEach(function(f){const warn=f.activate&&now<f.activate;
   ...
   else if(f.kind==='dragonBreath'){const active=now>=f.activate,spread=.22,length=CH,lanes=[{x:player.x-18,a:-Math.PI/2-spread},{x:player.x+18,a:-Math.PI/2+spread}];ctx.save();ctx.globalAlpha=active?.82:.25;ctx.lineCap='round';ctx.shadowColor='#ef4444';ctx.shadowBlur=active?18:0;lanes.forEach(function(lane){const x2=lane.x+Math.cos(lane.a)*length,y2=player.y-18+Math.sin(lane.a)*length;for(let layer=0;layer<3;layer++){ctx.strokeStyle=layer===0?'#dc2626':layer===1?'#f97316':'#fde047';ctx.lineWidth=[70,44,18][layer];ctx.beginPath();ctx.moveTo(lane.x,player.y-18);ctx.lineTo(x2,y2);ctx.stroke();}});ctx.restore();}
   ...
   else if(f.kind==='truffleDomain'){ctx.globalAlpha=.24;ctx.fillStyle='#fde047';ctx.shadowColor='#facc15';ctx.shadowBlur=16;ctx.beginPath();ctx.arc(f.x,f.y,60,0,Math.PI*2);ctx.fill();ctx.globalAlpha=.9;ctx.strokeStyle='#ffffff';ctx.lineWidth=4;ctx.setLineDash([10,7]);ctx.lineDashOffset=-now/28;ctx.stroke();ctx.setLineDash([]);for(let k=0;k<8;k++){const a=now/190+k*Math.PI/4,x=f.x+Math.cos(a)*42,y=f.y+Math.sin(a)*42;ctx.beginPath();ctx.moveTo(f.x,f.y);ctx.lineTo(x-4,y-6);ctx.lineTo(x+4,y);ctx.lineTo(x-4,y+6);ctx.stroke();}}
 ...
 useSkill1=function(){const cast=frV2Begin(1);if(!cast)return;const id=cast.id,c=cast.c,now=frV2Now();
  ...
  else if(id==='cocoa_popsicle_wargod'){const castId=now;[0,1000].forEach(function(d){setTimeout(function(){if(!gameRunning)return;frCocoaLaunchArc(d?1:-1);frV2Targets().forEach(function(t){if(frV2InFront(t,CH,CW*.375)){frV2SkillDamage(t,20,'skill');if(t._frV2CocoaFreezeCast!==castId){t._frV2CocoaFreezeCast=castId;t._frV2ParalyzedUntil=Math.max(t._frV2ParalyzedUntil||0,frV2Now()+(frV2Boss(t)?1000:2000));}frV2Slow(t,frV2Boss(t)?1000:2000,.05);}});},d);});}
  updateHUD();
 };
 ...
 useSkill2=function(){const cast=frV2Begin(2);if(!cast)return;const id=cast.id,now=frV2Now();
  ...
  else if(id==='dragonfruit_emperor')for(let i=0;i<2;i++)frV2SpawnMeteor(i*2000);
