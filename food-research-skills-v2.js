(function () {
  const specs = {
    ...
    garlic_knight:{passive:'殉戰蒜香：蒜頭騎士倒下時，使存活夥伴攻擊 +25%，持續 15 秒',skill1:'聖蒜淨化',skill1Cost:32,skill1Cooldown:9,skill1Desc:'解除全部異常狀態，並獲得 1.2 秒無敵。',skill2:'濃郁蒜味',skill2Cost:70,skill2Cooldown:20,skill2Desc:'展開 2.5 秒立場，反彈一般子彈並彈開小型敵人。'},
    ...
    salmon_ronin:{passive:'浪人步：閃避後攻擊與移速 +15%，持續 5 秒，每 8 秒最多觸發一次',skill1:'斷浪',skill1Cost:32,skill1Cooldown:9,skill1Desc:'揮出半畫面寬圓弧斬，造成 28 傷害、清除軌跡子彈並擊退敵人。',skill2:'逆流四閃',skill2Cost:65,skill2Cooldown:21,skill2Desc:'依序放出 4 道藍色直線刀光，每秒 1 道，每擊 38 傷害。'},
    ...
    puffer_alchemist:{passive:'劇毒調和：遠程 1%、近戰 3% 機率使敵人中毒並降低攻擊 5 秒',skill1:'毒沼',skill1Cost:30,skill1Cooldown:11,skill1Desc:'在角色上方生成 180 x 180 毒沼 7 秒，每秒造成 7 傷害。',skill2:'荊棘路',skill2Cost:70,skill2Cooldown:24,skill2Desc:'向畫面頂端生成荊棘路，困住一般敵人並附加 10 秒劇毒。'},
    black_garlic_void:{passive:'虛空穿刺：全隊忽略 20% 防禦並額外貫穿',skill1:'虛空黑洞',skill1Cost:50,skill1Cooldown:14,skill1Desc:'在畫面中心生成黑洞 1.5 秒，吸聚敵人與子彈後造成 25 傷害。',skill2:'雙鬼召來',skill2Cost:70,skill2Cooldown:22,skill2Desc:'召喚 2 隻不受敵方子彈傷害的鬼影作戰 10 秒，每 0.7 秒造成 12 傷害。'},
    ...
    truffle_thunder:{passive:'雷鏈麻痺：攻擊有 10% 機率連鎖，並短暫麻痺命中敵人',skill1:'雷環護身',skill1Cost:30,skill1Cooldown:13,skill1Desc:'環繞電流 10 秒，每 0.5 秒造成 10 傷害並彈開敵人。',skill2:'雷域推進',skill2Cost:70,skill2Cooldown:23,skill2Desc:'生成緩慢向上推進的雷電立場 6 秒，每 0.5 秒造成 16 傷害並短暫麻痺。'},
...
;(function(){
 ...
 function frV2UpdateFields(now){
  for(let i=frV2.fields.length-1;i>=0;i--){const f=frV2.fields[i];
   ...
   else if(f.kind==='truffleDomain'){f.y-=.85;if(now>=f.nextTick){f.nextTick=now+500;frV2Area(f.x,f.y,60,16,'skill',function(t){t._frV2ParalyzedUntil=Math.max(t._frV2ParalyzedUntil||0,now+(frV2Boss(t)?180:360));});}}
 ...
 function frV2UpdateSummons(now){
  for(let i=frV2.summons.length-1;i>=0;i--){const s=frV2.summons[i];if(now>=s.until||s.hp<=0){frV2.summons.splice(i,1);continue;}const t=frV2Nearest(s.x,s.y);
   if(s.kind==='bee'||s.kind==='ghost'||s.kind==='clone'){if(t){const d=frV2Distance(s,t)||1,want=s.kind==='ghost'?62:38;s.x+=(t.x-s.x)/d*(d>want?2.1:-1);s.y+=(t.y-s.y)/d*(d>want?2.1:-1);if(now>=s.nextAttack&&d<150){s.nextAttack=now+(s.kind==='clone'?650:700);frV2SkillDamage(t,s.damage,'summon');}}}
    ...
    for(let j=eBullets.length-1;j>=0;j--){const b=eBullets[j];if(frV2Distance(b,s)<16+(b.r||5)){if(s.kind!=='ghost')s.hp-=Math.max(1,b.dmg||5);eBullets.splice(j,1);break;}}
  }
 }
 ...
 function frV2Draw(){
  const now=frV2Now();ctx.save();ctx.lineWidth=3;frV2.fields.forEach(function(f){const warn=f.activate&&now<f.activate;
   ...
   else if(f.kind==='starchGuard'){const t=(now-f.start)/(f.until-f.start),r=50+Math.sin(now/80)*4;ctx.globalAlpha=Math.min(1,t*5)*Math.min(1,(1-t)*5)*.86;ctx.strokeStyle='#e7e5e4';ctx.fillStyle='rgba(214,211,209,.24)';ctx.lineWidth=5;for(let k=0;k<6;k++){const a=k*Math.PI/3,x=f.x+Math.cos(a)*r,y=f.y+Math.sin(a)*r;ctx.save();ctx.translate(x,y);ctx.rotate(a);ctx.beginPath();for(let q=0;q<6;q++){const aa=q*Math.PI/3,px=Math.cos(aa)*14,py=Math.sin(aa)*14;if(q===0)ctx.moveTo(px,py);else ctx.lineTo(px,py);}ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();}}
   else if(f.kind==='waterArc'){const t=Math.max(0,Math.min(1,(now-f.start)/(f.until-f.start))),y=f.y-f.r*t;ctx.globalAlpha=Math.sin(Math.PI*t);ctx.strokeStyle='#7dd3fc';ctx.shadowColor='#38bdf8';ctx.shadowBlur=16;ctx.lineWidth=30*(1-t*.35);ctx.beginPath();ctx.arc(f.x,y,f.r*.62,Math.PI*.12,Math.PI*.88);ctx.stroke();ctx.strokeStyle='#ffffff';ctx.lineWidth=5;ctx.stroke();}
   ...
   else if(f.kind==='truffleDomain'){ctx.globalAlpha=.3;ctx.fillStyle='#fde047';ctx.shadowColor='#facc15';ctx.shadowBlur=16;ctx.beginPath();ctx.arc(f.x,f.y,64,0,Math.PI*2);ctx.fill();ctx.globalAlpha=.95;ctx.strokeStyle='#ffffff';ctx.lineWidth=3;for(let k=0;k<6;k++){const a=now/150+k*Math.PI/3,x=f.x+Math.cos(a)*43,y=f.y+Math.sin(a)*43;ctx.beginPath();ctx.moveTo(f.x,f.y);ctx.lineTo(x-6,y-8);ctx.lineTo(x+5,y);ctx.lineTo(x-5,y+8);ctx.stroke();}}
 ...
 useSkill1=function(){const cast=frV2Begin(1);if(!cast)return;const id=cast.id,c=cast.c,now=frV2Now();
  ...
  else if(id==='onion_guard'){frClearBullets(165);frV2Area(player.x,player.y,165,20,'skill',function(t){frV2Push(t,player.x,player.y,55);});frV2Field('shockwave',{x:player.x,y:player.y,r:165,until:now+620});}
  ...
  else if(id==='potato_armor'){frV2Shield(35,70);frClearBullets(130);frV2Field('starchGuard',{x:player.x,y:player.y,follow:true,until:now+1100});}
  ...
  else if(id==='salmon_ronin'){frSlashFx(player.x,player.y,'#60a5fa',CW*.5,-Math.PI/2,600,0,28);frV2Field('waterArc',{x:player.x,y:player.y,r:CW*.5,until:now+720});frV2Targets().forEach(function(t){if(frV2InFront(t,CH,CW*.25)){frV2SkillDamage(t,28,'skill');frV2Push(t,player.x,player.y,80);}});for(let i=eBullets.length-1;i>=0;i--){if(frV2InFront(eBullets[i],CH,CW*.28))eBullets.splice(i,1);}}
