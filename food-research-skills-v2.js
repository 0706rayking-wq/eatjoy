...
;(function(){
 ...
 function frV2Draw(){
  const now=frV2Now();ctx.save();ctx.lineWidth=3;frV2.fields.forEach(function(f){const warn=f.activate&&now<f.activate;
   ...
   else if(f.kind==='iceSlashSprite'){const img=frV2EffectImage('iceSlash','cocoa-ice-slash.png'),t=Math.max(0,Math.min(1,(now-f.start)/(f.until-f.start))),w=Math.min(CW*.9,430),h=w*.56,y=f.y-t*105;ctx.save();ctx.globalAlpha=Math.sin(Math.PI*t);ctx.translate(f.x,y);ctx.rotate((f.side||1)*.05);ctx.scale(1,-1);if(img.complete&&img.naturalWidth)ctx.drawImage(img,-w/2,-h*.28,w,h);ctx.restore();}
 ...
 useSkill1=function(){const cast=frV2Begin(1);if(!cast)return;const id=cast.id,c=cast.c,now=frV2Now();
  ...
  else if(id==='cocoa_popsicle_wargod'){const castId=now;[0,1000].forEach(function(d){setTimeout(function(){if(!gameRunning)return;frV2Field('iceSlashSprite',{x:player.x,y:player.y-18,side:d?1:-1,until:frV2Now()+720});frSoundFx('ice',1);frV2Targets().forEach(function(t){if(frV2InFront(t,CH,CW*.375)){frV2SkillDamage(t,20,'skill');if(t._frV2CocoaFreezeCast!==castId){t._frV2CocoaFreezeCast=castId;t._frV2ParalyzedUntil=Math.max(t._frV2ParalyzedUntil||0,frV2Now()+(frV2Boss(t)?1000:2000));}frV2Slow(t,frV2Boss(t)?1000:2000,.05);}});},d);});}
  updateHUD();
 };
 ...
 useSkill2=function(){const cast=frV2Begin(2);if(!cast)return;const id=cast.id,now=frV2Now();
  ...
  else if(id==='cocoa_popsicle_wargod')frV2Field('icewall',{x:player.x,y:Math.max(85,player.y-105),w:330,h:50,hp:120,maxHp:120,until:now+10000});
  updateHUD();
 };
