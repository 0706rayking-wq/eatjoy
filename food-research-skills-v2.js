(function () {
  const specs = {
    ...
    healing_mushroom:{passive:'菌絲網絡：半徑 150 內敵方普通子彈速度 -20%、傷害 -10%',skill1:'採菇補給',skill1Cost:24,skill1Cooldown:7,skill1Desc:'在附近生成 3 朵蘑菇，每朵回復 15 HP，存在 7 秒。',skill2:'療癒菌林',skill2Cost:60,skill2Cooldown:18,skill2Desc:'回復 35 HP、獲得 45 護盾，並對周遭敵人造成 30 傷害。'},
    garlic_knight:{passive:'蒜影接力：每次切換角色時，在原位置留下持續 3 秒並自動攻擊的分身；結束後衝向新角色前方爆炸',skill1:'聖蒜淨化',skill1Cost:32,skill1Cooldown:9,skill1Desc:'解除全部異常狀態，並獲得 1.2 秒無敵。',skill2:'濃郁蒜味',skill2Cost:70,skill2Cooldown:20,skill2Desc:'展開 4 秒防護立場，反彈一般子彈並彈開小型敵人。'},
    ...
    octopus_samurai:{passive:'再生：3 秒未受傷後，每秒回復 2 HP',skill1:'噴灑墨汁',skill1Cost:24,skill1Cooldown:8,skill1Desc:'在上半場投下 2 灘墨汁 5 秒，使接觸敵人短暫停止攻擊。',skill2:'八刀流',skill2Cost:70,skill2Cooldown:24,skill2Desc:'召喚 8 支持刀觸手順時針環繞 10 秒，影響半徑 246；斬除一般子彈，命中造成 24 傷害並擊退，每次傷敵消耗 1 支觸手。'},
    ...
    black_garlic_void:{passive:'虛空換位：閃避時留下持續 8 秒的黑洞標記；已有標記時會留下新標記並瞬移到上一個標記',skill1:'虛空黑洞',skill1Cost:50,skill1Cooldown:14,skill1Desc:'在畫面中心生成黑洞 3 秒，持續吸聚敵人與子彈，結束後造成 25 傷害。',skill2:'雙鬼召來',skill2Cost:70,skill2Cooldown:22,skill2Desc:'召喚 2 隻 HP 70 的幽體鬼影 10 秒；登場 1 秒無敵，之後一般投射物傷害減半，穿過敵人時每 0.7 秒造成 30 傷害。'},
    ...
    peach_divine:{passive:'仙桃庇佑：兩名非操控且存活的夥伴每秒回復 1% 最大 HP；每場戰鬥只讓隊伍中首次死亡的 1 名角色以 30% HP 復活',skill1:'仙人模式',skill1Cost:45,skill1Cooldown:14,skill1Desc:'強化 6 秒，每 0.5 秒向前發射 1 道貫穿仙氣，共 12 道，每道 25 傷害。',skill2:'蟠桃回天',skill2Cost:100,skill2Cooldown:24,skill2Desc:'至少需要 100 氣力；施放時耗盡目前全部氣力，回復最大 HP 的 60%、解除異常、清除一般子彈，並無敵 3 秒。'},
...
;(function(){
 ...
 frV2EffectImage('iceSlash','cocoa-ice-slash.png');frV2EffectImage('iceWall','cocoa-ice-wall.png');frV2EffectImage('dragonFlame','dragonfruit-flame-4frame.png');frV2EffectImage('meteor','dragonfruit-meteor.png');frV2EffectImage('thornRoad','puffer-thorn-road.png');frV2EffectImage('octoBlade','octopus-samurai-blade-tentacle.png');
 ...
 Object.keys(FR_V2_SPEC).forEach(function(id){
  ...
  if(spec.name)form.name=spec.name;form.passive=spec.passive;form.skill1=spec.skill1;form.skill2=spec.skill2;form.skill1Desc=spec.skill1Desc;form.skill2Desc=spec.skill2Desc;form.skill1Cost=spec.skill1Cost;form.skill2Cost=spec.skill2Cost;form.skill1Cooldown=spec.skill1Cooldown;form.skill2Cooldown=spec.skill2Cooldown;
  const battle=FOOD_FORMS[id];if(battle){if(spec.name)battle.name=spec.name;battle.passive=spec.passive;battle.sk1Name=spec.skill1;battle.sk2Name=spec.skill2;battle.sk1Cd=spec.skill1Cooldown*1000;battle.sk2Cd=spec.skill2Cooldown*1000;}
 });
 ...
 function frV2Begin(level){const spec=frV2ActiveSpec(),peachReturn=level===2&&currentForm.id==='peach_divine',required=peachReturn?100:(level===1?spec.skill1Cost:spec.skill2Cost),cd=level===1?sk1Cd:sk2Cd;if(!gameRunning||stamina<required||cd>0)return null;stamina=peachReturn?0:Math.max(0,stamina-required);frV2SetCooldown(level,level===1?spec.skill1Cooldown:spec.skill2Cooldown);const name=level===1?spec.skill1:spec.skill2,c=currentForm.bulletColor||'#fbbf24';addText(name,player.x,player.y-38,c,16);frPlayCast(level,currentForm.id,c,name);frV2SignatureCue(currentForm.id,level,c);updateHUD();return{id:currentForm.id,c:c,spec:spec,skillMult:frSkillPowerMultiplier()};}
 ...
 function frV2ApplySharedPassives(now){
  ...
  if(frV2Has('healing_mushroom')){eBullets.forEach(function(b){if(b._frV2Mycelium||b.unblockable||b.special||b.bossSkill||b.frSpecial)return;if(frV2Distance(b,player)<=150+(b.r||5)){b._frV2Mycelium=true;b.vx*=.8;b.vy*=.8;b.dmg=Math.max(0,(b.dmg||0)*.9);frRingFx(b.x,b.y,'#86efac',18,240,2);}});}
  ...
  if(frV2Has('peach_divine')&&frV2Ready('peachBench',1000,now)){charSlots.forEach(function(ch,i){if(i!==activeChar&&ch&&ch.alive&&ch.hp<ch.maxHp)ch.hp=Math.min(ch.maxHp,ch.hp+ch.maxHp*.01);});updateHUD();}
 }
 ...
 function frV2HitEffects(target,amount){
  ...
  if(frV2Has('cocoa_popsicle_wargod')&&((target.frozenTimer||0)>0||(target._frSlowUntil||0)>now))amount*=1.2;
 ...
 hurtPlayer=function(raw){
  ...
  const hp=player.hp,shield=player.shieldHp||0,damagedSlot=charSlots&&charSlots[activeChar],hitX=player.x,hitY=player.y,r=frV2BaseHurt(dmg),remainingHp=damagedSlot?Math.max(0,damagedSlot.hp):player.hp,lost=Math.max(0,hp-remainingHp),now=frV2Now();if(lost>0||(player.shieldHp||0)<shield)frV2.lastDamageAt=now;
  const ch=damagedSlot;if(lost>0&&ch&&frV2Has('potato_armor')){ch._frV2SproutLoss=(ch._frV2SproutLoss||0)+lost;const threshold=Math.max(1,ch.maxHp*.25);if(ch._frV2SproutLoss>=threshold&&now>=(ch._frV2SproutReadyAt||0)){ch._frV2SproutLoss-=threshold;ch._frV2SproutReadyAt=now+10000;const angle=Math.random()*Math.PI*2,dist=55+Math.random()*35,x=Math.max(24,Math.min(CW-24,hitX+Math.cos(angle)*dist)),y=Math.max(75,Math.min(CH-35,hitY+Math.sin(angle)*dist));frV2Pickup('lifeSprout',x,y,now+6000,{healPct:.05});frRingFx(x,y,'#a3e635',34,520,4);addText('生命嫩芽',x,y-22,'#a3e635',11,-.35);}}
  return r;
 };
 const frV2BaseDodge=doDodge;
 doDodge=function(){const was=player.dodging,startX=player.x,startY=player.y,r=frV2BaseDodge.apply(this,arguments);if(!was&&player.dodging){const now=frV2Now();if(frV2Has('salmon_ronin')&&now>=frV2.salmonReadyAt){frV2.salmonBuffUntil=now+5000;frV2.salmonReadyAt=now+8000;addText('浪人步',player.x,player.y-34,'#60a5fa',12);}if(frV2Has('black_garlic_void')){const old=frV2.fields.find(function(f){return f.kind==='voidAnchor'&&now<f.until;});if(old){const tx=old.x,ty=old.y;old.until=0;frV2Field('voidAnchor',{x:startX,y:startY,r:34,until:now+8000});frBoltFx(startX,startY,tx,ty,'#a78bfa',360,0,7);frV2Burst(startX,startY,'#7c3aed',12);player.x=Math.max(player.radius,Math.min(CW-player.radius,tx));player.y=Math.max(player.radius+45,Math.min(CH-player.radius,ty));frV2Burst(player.x,player.y,'#c4b5fd',16);addText('虛空換位',player.x,player.y-34,'#c4b5fd',12,-.4);}else{frV2Field('voidAnchor',{x:startX,y:startY,r:34,until:now+8000});frRingFx(startX,startY,'#8b5cf6',42,500,5);addText('黑洞標記',startX,startY-30,'#a78bfa',11,-.35);}}}return r;};

 function frV2UpdateFields(now){
  for(let i=frV2.fields.length-1;i>=0;i--){const f=frV2.fields[i];
   if(now>=f.until){if(f.kind==='cheese'&&!f.exploded){frV2Area(f.x,f.y,140,50,'skill');frV2Burst(f.x,f.y,'#facc15',22);}else if(f.kind==='blackhole'&&!f.exploded){f.exploded=true;for(let j=eBullets.length-1;j>=0;j--)if(frV2Distance(eBullets[j],f)<115)eBullets.splice(j,1);frV2Area(f.x,f.y,115,25,'skill');frV2Burst(f.x,f.y,'#a855f7',24);}else if(f.kind==='garlicClone'&&!f.exploded){f.exploded=true;const ex=player.x,ey=Math.max(70,player.y-95);frSlashFx(f.x,f.y,'#e9d5ff',Math.hypot(ex-f.x,ey-f.y),Math.atan2(ey-f.y,ex-f.x),380,0,16);frV2Area(ex,ey,100,35,'skill',function(t){frV2Push(t,ex,ey,75);},f.skillMult);frV2Burst(ex,ey,'#e9d5ff',24);frGroundFx(ex,ey,'#c084fc',100,620);}frV2.fields.splice(i,1);continue;}
   if(f.kind==='chili'&&now>=f.nextTick){f.nextTick=now+500;frV2Area(f.x,f.y,f.r,9,'dot',function(t){frV2Burn(t,2200,9);});}
    ...
    else if(f.kind==='tentacle'){for(let j=eBullets.length-1;j>=0;j--){const b=eBullets[j];if(frV2Distance(b,player)<246+(b.r||5)){frV2Burst(b.x,b.y,'#e9d5ff',4);eBullets.splice(j,1);}}if((f.blades||0)>0&&now>=f.nextTick){f.nextTick=now+550;const target=frV2Targets().filter(function(t){return frV2Distance(t,player)<=246+(t.r||18);}).sort(function(a,b){return frV2Distance(a,player)-frV2Distance(b,player);})[0];if(target){frV2SkillDamage(target,24,'skill',f.skillMult);frV2Push(target,player.x,player.y,frV2Boss(target)?45:110);f.blades--;frV2Burst(target.x,target.y,'#e9d5ff',12);frSlashFx(player.x,player.y,'#f5d0fe',frV2Distance(target,player),Math.atan2(target.y-player.y,target.x-player.x),420,0,18);if(f.blades<=0)f.until=Math.min(f.until,now+420);}}}
    else if(f.kind==='garlicClone'&&now>=f.nextTick){f.nextTick=now+600;const t=frV2Nearest(f.x,f.y);if(t){const a=Math.atan2(t.y-f.y,t.x-f.x),b=new Bullet(f.x,f.y,Math.cos(a)*9,Math.sin(a)*9,10*(window._curAtkMult||atkMult),'#e9d5ff',6,true,false,false);b.frV2Qi=true;bullets.push(b);frSlashFx(f.x,f.y,'#d8b4fe',34,a,260,0,6);}}
   ...
   else if(f.kind==='meteor'&&now>=f.activate&&!f.exploded){f.exploded=true;f.explodedAt=now;frV2Targets().forEach(function(t){const d=frV2Distance(t,f);if(d<=f.r+(t.r||18)){frV2SkillDamage(t,d<=f.r*.45?100:75,'skill',f.skillMult);frV2Burn(t,3500,16);if(frV2Boss(t)){if(now>=(t._frV2MeteorStunReady||0)){t._frV2MeteorStunReady=now+3000;t._frV2ParalyzedUntil=Math.max(t._frV2ParalyzedUntil||0,now+1000);}}else t._frV2ParalyzedUntil=Math.max(t._frV2ParalyzedUntil||0,now+1500);}});frV2Burst(f.x,f.y,'#fb923c',35);frShakeFx(12,520);frFlashFx('#f97316',320,.26);}
 ...
 function frV2UpdateSummons(now){
  for(let i=frV2.summons.length-1;i>=0;i--){const s=frV2.summons[i];if(now>=s.until||s.hp<=0){frV2.summons.splice(i,1);continue;}const t=frV2Nearest(s.x,s.y);
   if(s.kind==='bee'&&t){const count=Math.max(1,s.formationCount||5),angle=(s.formationIndex||0)*Math.PI*2/count+now/1800,orbitX=t.x+Math.cos(angle)*42,orbitY=t.y+Math.sin(angle)*30,dSlot=Math.hypot(orbitX-s.x,orbitY-s.y)||1,d=frV2Distance(s,t)||1;s.x+=(orbitX-s.x)/dSlot*Math.min(2.1,dSlot*.12);s.y+=(orbitY-s.y)/dSlot*Math.min(2.1,dSlot*.12);frV2.summons.forEach(function(other){if(other===s||other.kind!=='bee')return;const dx=s.x-other.x,dy=s.y-other.y,bd=Math.hypot(dx,dy)||1;if(bd<26){const push=(26-bd)*.045;s.x+=dx/bd*push;s.y+=dy/bd*push;}});if(now>=s.nextAttack&&d<58+(t.r||18)){s.nextAttack=now+700;frV2SkillDamage(t,s.damage,'summon',s.skillMult);frV2Burst(t.x,t.y,'#fde047',7);frRingFx(t.x,t.y,'#fef3c7',30,260,4);if(!frV2Boss(t))frV2Push(t,s.x,s.y,16);}}
 ...
 function frV2Draw(){
  const now=frV2Now();ctx.save();ctx.lineWidth=3;frV2.fields.forEach(function(f){const warn=f.activate&&now<f.activate;
   ...
   else if(f.kind==='garlic'){const r=130;ctx.strokeStyle='#e9d5ff';ctx.shadowColor=ctx.strokeStyle;ctx.shadowBlur=12;ctx.beginPath();ctx.arc(player.x,player.y,r,0,Math.PI*2);ctx.stroke();}
   else if(f.kind==='magneticLane'){const pulse=.9+.1*Math.sin(now/70);ctx.save();ctx.lineCap='round';ctx.shadowColor='#fde047';ctx.shadowBlur=16;ctx.globalAlpha=.24;ctx.strokeStyle='#facc15';ctx.lineWidth=48;ctx.beginPath();ctx.moveTo(f.x1,f.y1);ctx.lineTo(f.x2,f.y2);ctx.stroke();ctx.globalAlpha=.95;ctx.strokeStyle='#fff7ae';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(f.x1,f.y1);for(let k=1;k<8;k++){const q=k/8,x=f.x1+(f.x2-f.x1)*q,y=f.y1+(f.y2-f.y1)*q+Math.sin(now/45+k*2.3)*9;ctx.lineTo(x,y);}ctx.lineTo(f.x2,f.y2);ctx.stroke();[[-1,f.x1,f.y1],[1,f.x2,f.y2]].forEach(function(p){ctx.globalAlpha=.88;ctx.fillStyle=p[0]<0?'#60a5fa':'#fde047';ctx.strokeStyle='#ffffff';ctx.lineWidth=3;ctx.beginPath();ctx.arc(p[1],p[2],22*pulse,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.globalAlpha=.65;ctx.strokeStyle=p[0]<0?'#93c5fd':'#fef08a';ctx.lineWidth=4;ctx.beginPath();ctx.arc(p[1],p[2],30+pulse*4,now/240,now/240+Math.PI*1.4);ctx.stroke();});ctx.restore();}
   else if(f.kind==='tentacle'){const img=frV2EffectImage('octoBlade','octopus-samurai-blade-tentacle.png'),count=Math.max(0,f.blades||0),base=now/430;for(let k=0;k<count;k++){const a=base+k*Math.PI*2/Math.max(1,count),len=150+Math.sin(now/170+k)*10;ctx.save();ctx.translate(player.x+Math.cos(a)*len*.46,player.y+Math.sin(a)*len*.46);ctx.rotate(a-.58);ctx.globalAlpha=.94;ctx.shadowColor='#7e22ce';ctx.shadowBlur=10;if(img.complete&&img.naturalWidth)ctx.drawImage(img,-18,-42,210,140);else{ctx.strokeStyle='#c084fc';ctx.lineWidth=16;ctx.beginPath();ctx.moveTo(-65,0);ctx.lineTo(105,0);ctx.stroke();}ctx.restore();}}
   else if(f.kind==='garlicClone'){const t=Math.max(0,Math.min(1,(now-f.start)/(f.until-f.start))),rush=t>.84?(t-.84)/.16:0,x=f.x+(player.x-f.x)*rush,y=f.y+(Math.max(70,player.y-95)-f.y)*rush,img=typeof frImg==='function'?frImg(f.formId||'garlic_knight','battle'):null,sheet=img&&img.complete&&img.naturalWidth&&typeof frBuildBattleSheet==='function'?frBuildBattleSheet(f.formId||'garlic_knight',img):null;ctx.save();ctx.translate(x,y);ctx.globalAlpha=.48+.22*Math.sin(now/90);ctx.shadowColor='#c084fc';ctx.shadowBlur=14;if(sheet){const frame=Math.floor(now/145)%4;ctx.drawImage(sheet,frame*FR_BATTLE_CELL,0,FR_BATTLE_CELL,FR_BATTLE_CELL,-29,-35,58,58);}else{ctx.fillStyle='#e9d5ff';ctx.beginPath();ctx.arc(0,0,24,0,Math.PI*2);ctx.fill();}ctx.restore();}
   else if(f.kind==='voidAnchor'){const pulse=.86+.14*Math.sin(now/100);ctx.save();ctx.translate(f.x,f.y);ctx.globalAlpha=.72;ctx.fillStyle='#111827';ctx.shadowColor='#7c3aed';ctx.shadowBlur=18;ctx.beginPath();ctx.arc(0,0,24*pulse,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#c4b5fd';ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,0,32*pulse,now/350,now/350+Math.PI*1.55);ctx.stroke();ctx.restore();}
   ...
   else if(f.kind==='meteor'&&now>=f.launch){const t=Math.max(0,Math.min(1,(now-f.launch)/(f.activate-f.launch))),fall=1-Math.pow(1-t,2.25),mx=f.x,my=f.fromY+(f.y-f.fromY)*fall;if(!f.exploded){ctx.globalAlpha=.2+.12*Math.sin(now/70);ctx.fillStyle='#ef4444';ctx.beginPath();ctx.arc(f.x,f.y,f.r,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#fde047';ctx.setLineDash([10,7]);ctx.stroke();ctx.setLineDash([]);const img=frV2EffectImage('meteor','dragonfruit-meteor.png'),w=f.r*1.45,h=f.r*2.18;ctx.save();ctx.globalAlpha=.98;ctx.translate(mx,my);ctx.rotate(.58);ctx.shadowColor='#f97316';ctx.shadowBlur=14;if(img.complete&&img.naturalWidth)ctx.drawImage(img,-w/2,-h*.82,w,h);ctx.restore();}else{const e=Math.max(0,Math.min(1,(now-(f.explodedAt||f.activate))/720));ctx.globalAlpha=1-e*.82;const rg=ctx.createRadialGradient(f.x,f.y,4,f.x,f.y,f.r*(.5+e));rg.addColorStop(0,'#ffffff');rg.addColorStop(.25,'#fde047');rg.addColorStop(.62,'#f97316');rg.addColorStop(1,'rgba(127,29,29,0)');ctx.fillStyle=rg;ctx.beginPath();ctx.arc(f.x,f.y,f.r*(.55+e*.75),0,Math.PI*2);ctx.fill();ctx.strokeStyle='#ffffff';ctx.lineWidth=8*(1-e*.6);ctx.stroke();for(let k=0;k<10;k++){const a=k*Math.PI/5,len=f.r*(.7+e*1.25);ctx.beginPath();ctx.moveTo(f.x+Math.cos(a)*f.r*.25,f.y+Math.sin(a)*f.r*.25);ctx.lineTo(f.x+Math.cos(a)*len,f.y+Math.sin(a)*len);ctx.stroke();}}}
   else if(f.kind==='icewall'){const img=frV2EffectImage('iceWall','cocoa-ice-wall.png'),visualH=Math.max(105,f.h*2.2);ctx.save();ctx.globalAlpha=.94;ctx.shadowColor='#38bdf8';ctx.shadowBlur=12;if(img.complete&&img.naturalWidth)ctx.drawImage(img,f.x-f.w/2,f.y-visualH*.72,f.w,visualH);ctx.restore();ctx.fillStyle='rgba(15,23,42,.78)';ctx.fillRect(f.x-f.w/2,f.y+f.h/2+7,f.w,5);ctx.fillStyle='#67e8f9';ctx.fillRect(f.x-f.w/2,f.y+f.h/2+7,f.w*Math.max(0,f.hp/f.maxHp),5);}
  ...
  if(frV2Has('healing_mushroom')){ctx.save();const r=150,pulse=.96+.025*Math.sin(now/180);ctx.globalAlpha=.1;ctx.fillStyle='#86efac';ctx.beginPath();ctx.arc(player.x,player.y,r,0,Math.PI*2);ctx.fill();ctx.globalAlpha=.48;ctx.strokeStyle='#bbf7d0';ctx.lineWidth=2;ctx.setLineDash([5,9]);ctx.lineDashOffset=-now/55;ctx.beginPath();ctx.arc(player.x,player.y,r*pulse,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.globalAlpha=.28;for(let ring=1;ring<=2;ring++){ctx.beginPath();ctx.arc(player.x,player.y,r*ring/3,0,Math.PI*2);ctx.stroke();}for(let k=0;k<10;k++){const a=k*Math.PI/5+Math.sin(now/700+k)*.08;ctx.beginPath();ctx.moveTo(player.x+Math.cos(a)*r*.15,player.y+Math.sin(a)*r*.15);ctx.lineTo(player.x+Math.cos(a+.18)*r*.96,player.y+Math.sin(a+.18)*r*.96);ctx.stroke();}ctx.restore();}
  ...
  frV2.pickups.forEach(function(p){ctx.save();ctx.translate(p.x,p.y);if(p.kind==='lifeSprout'){const sway=Math.sin(now/180+p.x)*.12;ctx.rotate(sway);ctx.shadowColor='#84cc16';ctx.shadowBlur=12;ctx.strokeStyle='#4d7c0f';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(0,13);ctx.quadraticCurveTo(-2,1,0,-12);ctx.stroke();ctx.fillStyle='#a3e635';ctx.beginPath();ctx.ellipse(-8,-6,10,5,-.45,0,Math.PI*2);ctx.ellipse(8,-10,10,5,.45,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#ecfccb';ctx.lineWidth=2;ctx.stroke();ctx.globalAlpha=.45+.18*Math.sin(now/110);ctx.strokeStyle='#bef264';ctx.beginPath();ctx.arc(0,1,22,0,Math.PI*2);ctx.stroke();}else{ctx.shadowColor='#86efac';ctx.shadowBlur=10;ctx.fillStyle='#f0fdf4';ctx.fillRect(-3,0,6,12);ctx.fillStyle='#86efac';ctx.beginPath();ctx.arc(0,0,13,Math.PI,Math.PI*2);ctx.fill();ctx.fillStyle='#ffffff';for(let k=0;k<3;k++){ctx.beginPath();ctx.arc(-6+k*6,-4-(k%2)*3,2,0,Math.PI*2);ctx.fill();}}ctx.restore();});
 ...
 function frV2Tick(now){
  ...
  for(let i=frV2.pickups.length-1;i>=0;i--){const p=frV2.pickups[i];if(now>p.until)frV2.pickups.splice(i,1);else if(frV2Distance(p,player)<34){frV2Heal(p.kind==='lifeSprout'?player.maxHp*(p.healPct||.05):15);frV2.pickups.splice(i,1);}}
 ...
 useSkill2=function(){const cast=frV2Begin(2);if(!cast)return;const id=cast.id,now=frV2Now();
  ...
  else if(id==='honey_priest'){frV2Field('honey',{x:player.x,y:player.y-45,r:156,until:now+8000});for(let i=0;i<5;i++){const angle=-Math.PI/2+i*Math.PI*2/5;frV2Summon('bee',{x:player.x+Math.cos(angle)*54,y:player.y+Math.sin(angle)*38,hp:35,maxHp:35,damage:16,skillMult:cast.skillMult,until:now+12000,start:now,formationIndex:i,formationCount:5});}}
  ...
  else if(id==='octopus_samurai')frV2Field('tentacle',{blades:8,r:246,skillMult:cast.skillMult,nextTick:now+180,until:now+10000});
  ...
  else if(id==='peach_divine'){frV2Heal(player.maxHp*.6);frCleanse();frClearBullets(Infinity);player.invTimer=Math.max(player.invTimer,180);frV2Aura('divine','#f9a8d4',3000,{r:118});}
