(function () {
  const specs = {
    ...
    peach_divine:{passive:'仙桃庇佑：後排夥伴每秒回復 2% 最大 HP，全隊首次死亡時以 30% HP 復活',skill1:'仙人模式',skill1Cost:45,skill1Cooldown:14,skill1Desc:'強化 6 秒，每 0.5 秒發射 1 道貫穿仙氣，共 12 道，每道 18 傷害。',skill2:'蟠桃回天',skill2Cost:150,skill2Cooldown:16,skill2Desc:'完全回復 HP、解除異常、清除一般子彈，並無敵 5 秒。'},
...
;(function(){
 ...
 function frV2ApplySharedPassives(now){
  ...
  if(frV2Has('peach_divine')&&frV2Ready('peachBench',1000,now)){charSlots.forEach(function(ch,i){if(i!==activeChar&&ch&&ch.alive&&ch.hp<ch.maxHp)ch.hp=Math.min(ch.maxHp,ch.hp+ch.maxHp*.02);});updateHUD();}
 }
