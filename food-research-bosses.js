...
const FR_BOSS_SKILL_ASSET_FILES={
  jellyClones:'jelly-clones.png',crownSlam:'jelly-crown-slam.png',sandwormBurrow:'sandworm-burrow.png',sandwormDevour:'sandworm-devour.png',
  chickenLegRain:'cream-chicken-rain.png',creamBaptism:'cream-baptism.png',mechMissile:'heavy-mech-missile.png',pressureCannon:'heavy-mech-pressure-cannon.png',
  ...
  durianIceLance:'durian-ice-lance.png',frostThornWheel:'durian-frost-wheel.png',mangoSnowcone:'mango-snowcone.png',syrupAvalanche:'mango-syrup-avalanche.png',
  friesIaido:'fries-iaido.png',swordNet:'fries-sword-net.png',noodleBind:'pasta-noodle-bind.png',noodleBindGrid:'pasta-bind-grid.png',
  soupGeysers:'pastry-soup-geyser.png',pastryCollapse:'pastry-collapse.png',inkEightThrust:'octopus-ink-thrust.png',eightArmCombo:'octopus-eight-arm.png'
};
...
function frBossSkillAssetFx(b,pattern){
  ...
  if(!base||!b)return;
  const f=Object.assign({},base,{color:b.color||'#fef08a',follow:!!base.follow});
  ...
  for(let i=frBossFx.length-1;i>=0;i--){
    ...
    if(f.kind==='asset'){
      ...
      if(img&&img.complete&&img.naturalWidth){
        ...
        const fit=Math.max(width/bounds.w,height/bounds.h),renderW=bounds.w*fit,renderH=bounds.h*fit;
...
function frBossCast(b,pattern,isSkill){
  ...
  else if(pattern==='jellyClones'){frBossFxPush(b,'asset',{asset:'jellyClones',motion:'bounce',x:b.x,y:b.y+28,follow:false,duration:72,drawW:176,drawH:117,toY:96,pulse:.05});frBossLater(b,w,function(){frBossSummonMinions(b,2,1);});for(let s=-1;s<=1;s++)frBossLater(b,w+s*8,function(){frBossFan({x:b.x+s*70,y:b.y,color:c},3,4,d,.22,c);});}
  ...
  else if(pattern==='cleaverLanes'){const gap=Math.floor(Math.random()*4);frBossLater(b,w,function(){for(let i=0;i<5;i++){if(i===gap||i===gap+1)continue;frBossHazard({kind:'rect',x:(i+.5)*CW/5,y:CH/2,w:Math.min(CW/5.5,68),h:CH,color:c,damage:d,delay:90+(i>gap?i-2:i)*12,minDelay:90,duration:24});}});}
  ...
  else if(pattern==='chickenLegRain'){frBossFxPush(b,'asset',{asset:'chickenLegRain',x:CW*.5,y:30,follow:false,duration:76,drawW:190,drawH:127,fromY:-100,toY:CH*.72,rotate:.18});frBossLater(b,w,function(){frBossSummonMinions(b,2,3);});frBossMeteor(b,8,c,d,true);}
  ...
  else if(pattern==='crownSlam'){for(let q=0;q<3;q++)frBossLater(b,q*48,function(){const x=player.x+(player.vx||0)*12,y=player.y+(player.vy||0)*12;frBossFxPush(b,'asset',{asset:'crownSlam',motion:'fall',x:x,y:y,follow:false,duration:68,drawW:190+q*14,drawH:199+q*15,pulse:.04});frBossHazard({kind:'circle',x:x,y:y,r:68+q*7,color:c,damage:d,delay:52,duration:18});frBossLater(b,34,function(){if(!b._defeated){b.x=Math.max(60,Math.min(CW-60,x));b.y=Math.max(frBossSafeCenterY(b),Math.min(CH*.52,y));}});});}
  ...
  else if(pattern==='missileLock'){for(let q=0;q<4;q++)frBossLater(b,q*28,function(){const x=player.x+(player.vx||0)*14,y=player.y+(player.vy||0)*14,a=Math.atan2(y-b.y,x-b.x);frBossFxPush(b,'asset',{asset:'mechMissile',x:b.x,y:b.y,follow:false,duration:62,drawW:112,drawH:75,toX:x-b.x,toY:y-b.y,rotate:a+Math.PI,anchorX:.64});frBossHazard({kind:'circle',x:x,y:y,r:48+q*3,color:'#22d3ee',damage:d,delay:48,duration:17});});}
  ...
  else if(pattern==='burrowAmbush'){const tx=player.x+(player.vx||0)*18,ty=player.y+(player.vy||0)*18,a=Math.atan2(ty-b.y,tx-b.x),reach=Math.max(CW,CH)*1.35;frBossFxPush(b,'asset',{asset:'sandwormDevour',motion:'rise',x:tx,y:ty,follow:false,duration:96,drawW:230,drawH:230,anchorY:.72,spin:Math.PI*.45,pulse:.05});frBossHazard({kind:'line',x:b.x,y:b.y,x2:b.x+Math.cos(a)*reach,y2:b.y+Math.sin(a)*reach,w:38,color:c,damage:d,status:{kind:'poison',duration:3600},delay:50,duration:20});frBossLater(b,52,function(){b.x=Math.max(55,Math.min(CW-55,tx));b.y=Math.max(frBossSafeCenterY(b),Math.min(CH*.58,ty));frBossHazard({kind:'circle',x:b.x,y:b.y,r:78,color:c,damage:d,delay:34,minDelay:24,duration:18});});}
  ...
  else if(pattern==='creamBaptism'){frBossFxPush(b,'asset',{asset:'creamBaptism',x:CW*.2,y:CH*.42,follow:false,duration:174,drawW:CW*.38,drawH:CH*.9,fromY:-CH*.3,toY:0,alpha:.72});frBossFxPush(b,'asset',{asset:'creamBaptism',x:CW*.8,y:CH*.42,follow:false,duration:174,drawW:CW*.38,drawH:CH*.9,fromY:-CH*.3,toY:0,alpha:.72});const maxWidth=Math.max(72,(CW-frBossSafeCorridor()-player.radius*2)/2);for(let q=0;q<4;q++){const width=Math.min(maxWidth,54+q*24),delay=90+q*24;frBossHazard({kind:'rect',x:width/2,y:CH/2,w:width,h:CH,color:c,damage:d,status:{kind:'attackDown',duration:3000},delay:delay,minDelay:90,duration:16});frBossHazard({kind:'rect',x:CW-width/2,y:CH/2,w:width,h:CH,color:c,damage:d,status:{kind:'attackDown',duration:3000},delay:delay,minDelay:90,duration:16});}}
  ...
  else if(pattern==='pressureCannon'){for(let q=0;q<3;q++)frBossLater(b,q*44,function(){const a=frBossAim(b,true),reach=Math.max(CW,CH)*1.55;frBossFxPush(b,'asset',{asset:'pressureCannon',x:b.x,y:b.y,follow:false,duration:64-q*5,drawW:Math.min(440,reach*.72),drawH:92-q*10,rotate:a+Math.PI,anchorX:.92,pulse:.04,alpha:.88});frBossHazard({kind:'line',x:b.x,y:b.y,x2:b.x+Math.cos(a)*reach,y2:b.y+Math.sin(a)*reach,w:42-q*8,color:'#67e8f9',damage:d,status:q===2?{kind:'slow',duration:2400}:null,delay:48-q*7,minDelay:24,duration:16});});}
