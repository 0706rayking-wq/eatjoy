...
function frThunderDrawHandSprite(img,timer,x,y,w,h){if(!img||!img.complete||!img.naturalWidth)return false;const frame=Math.floor((Number(timer)||0)/8)%4,cellW=img.naturalWidth/4;ctx.drawImage(img,frame*cellW,0,cellW,img.naturalHeight,x,y,w,h);return true;}
...
function frThunderSpawnHands(b){
  ...
  frThunderRelicImage('leftHand');frThunderRelicImage('rightHand');frThunderRelicImage('leftHandAnim');frThunderRelicImage('rightHandAnim');
}
...
const FR_THUNDER_SUPPORT_THRESHOLDS=[.8,.6,.4,.2];
const FR_THUNDER_SUPPORT_ENTRY_LINES=[
  '撐住，我們從另一間分店趕來了！','別一個人扛，接下來換我們並肩。','聽見雷聲了嗎？那是我們來支援的信號。','你已經走得夠遠，最後這段路一起走。',
  '店裡的大家都在看著，別在這裡停下。','我們不是來搶你的勝利，是來守住你的背後。','跨過這片雷光，我們一起回營地。','你負責打倒小雷神，雙手交給我們。',
  '別怕，其他分店也站在你這邊。','你的戰鬥傳到我們店裡了，現在輪到我們回應。','還能握住武器嗎？能的話就和我們一起上。','我們帶來的不多，只有火力和不肯放棄。',
  '不是只有你想看見這場決戰的終點。','前面的路你自己闖過，最後一程讓我們陪。','雷再大也蓋不過大家替你加油的聲音。','你沒有孤軍奮戰，從來都沒有。',
  '援軍抵達，先拆掉那雙麻煩的手！','別回頭，我們會替你擋住這一擊。','今天不分哪間店，我們都是同一隊。','把呼吸穩住，勝利已經比剛才更近。',
  '你守住了希望，現在換希望來守你。','我們會把能做的都留在這裡。','還沒結束，大家的力量才剛到齊。','來吧，讓小雷神看看全台接力的厲害。'
];
const FR_THUNDER_SUPPORT_GUARD_LINES=[
  '這一擊我來擋！','別停下，繼續攻擊！','你的血量比我的重要。','放心，背後交給我。','還不到倒下的時候！','雷光過來了，躲在我後面！',
  '我接得住！','別為我分心，瞄準雙手！','這次換我保護你。','只要你還站著，我們就有機會。','傷害由我承擔！','往前走，不要回頭！'
];
const FR_THUNDER_SUPPORT_LAST_LINES=[
  '我的力量到這裡了，但你的路還沒走完。','別替我停下，帶著這一招繼續戰鬥。','能並肩到現在，已經是我最驕傲的一戰。','我先退場，剩下的勇氣全都交給你。',
  '最後一點力量，拿去突破那道雷光吧。','我們不是消失，只是把位置讓給下一位夥伴。','答應我，回營地時要告訴大家你贏了。','我撐不住了，但我相信你撐得住。',
  '別難過，能替你擋下這一擊就值得了。','我的攻擊會留下，陪你打到最後。','這不是告別，是把接力棒交到你手上。','看著前方，勝利正在那裡等你。',
  '如果害怕，就想起還有很多人在替你加油。','我能做的都做了，接下來就看你的了。','最後這招不算漂亮，但一定能幫上忙。','別讓小雷神看見你低頭。',
  '我先回去準備慶功，你可別讓我白忙。','就算只剩一個人，也帶著所有人的心意戰鬥。','下一道雷來時，記得我們曾替你擋過。','你已經證明自己能走到這裡，再多走一步就好。',
  '我們留下的不是犧牲，是讓你繼續前進的空間。','請替我們完成這場決戰。','最後的火力，全數交付！','去吧，讓雷霆之境記住你的名字。'
];
const FR_THUNDER_SUPPORT_FALLBACK=[
  {name:'北區的夥伴',branch:'北區',formId:'cocoa_popsicle_wargod'},{name:'中區的夥伴',branch:'中區',formId:'dragonfruit_emperor'},
  {name:'南區的夥伴',branch:'南區',formId:'honey_priest'},{name:'東區的夥伴',branch:'東區',formId:'lemon_battery'},
  {name:'海線的夥伴',branch:'海線',formId:'black_garlic_void'},{name:'山城的夥伴',branch:'山城',formId:'potato_armor'},
  {name:'港區的夥伴',branch:'港區',formId:'peach_divine'},{name:'城中的夥伴',branch:'城中',formId:'salmon_ronin'}
];
const FR_THUNDER_SUPPORT_FORMS=['onion_guard','popcorn','healing_mushroom','garlic_knight','chili_sprite','lotus_archer','potato_armor','lemon_battery','cheese_mage','honey_priest','coffee_pilot','octopus_samurai','salmon_ronin','puffer_alchemist','black_garlic_void','lobster_general','truffle_thunder','dragonfruit_emperor','peach_divine','cocoa_popsicle_wargod'];
const FR_THUNDER_SUPPORT_ULTIMATES={onion_guard:'三層洋蔥盾',popcorn:'連發爆米砲',healing_mushroom:'療癒菌林',garlic_knight:'濃郁蒜味',chili_sprite:'麻辣風暴',lotus_archer:'二十發貫穿箭',potato_armor:'澱粉分裂',lemon_battery:'電磁巨砲',cheese_mage:'起司誘餌',honey_priest:'蜜語結界',coffee_pilot:'子彈時間',octopus_samurai:'八爪章魚',salmon_ronin:'逆流多重閃',puffer_alchemist:'荊棘路',black_garlic_void:'雙鬼召來',lobster_general:'雙側砲台',truffle_thunder:'雷域推進',dragonfruit_emperor:'龍隕五星',peach_divine:'蟠桃回天',cocoa_popsicle_wargod:'冰霜劍氣'};
function frThunderSupportLine(pool,index){return pool[Math.abs(index)%pool.length];}
function frThunderSupportPool(){const source=typeof THUNDER_SUPPORTERS!=='undefined'&&Array.isArray(THUNDER_SUPPORTERS)?THUNDER_SUPPORTERS:[];return (source.length?source:FR_THUNDER_SUPPORT_FALLBACK).slice(0,8);}
function frThunderSupportSpeak(s,line,color){if(typeof frActorSpeak==='function')frActorSpeak((s&&s.name?s.name:'援軍')+'：「'+line+'」',color||'#fef08a');else addText(line,CW/2,115,color||'#fef08a',15,-.3);}
function frThunderSupportInit(b){
  if(b._frSupportReady)return;b._frSupportReady=true;b._frSupportNext=0;b._frSupportQueue=[];b._frSupporters=[];b._frSupportShots=[];b._frSupportPool=frThunderSupportPool();b._frSupportFormDeck=frBossShuffle(FR_THUNDER_SUPPORT_FORMS);b._frSupportSerial=0;
}
function frThunderSupportSpawnWave(b,mark){
  const pool=b._frSupportPool||FR_THUNDER_SUPPORT_FALLBACK,wave=[];
  for(let i=0;i<2;i++){const supportIndex=b._frSupportSerial++,raw=pool[supportIndex%pool.length]||FR_THUNDER_SUPPORT_FALLBACK[i],side=i?1:-1,maxHp=100,formId=b._frSupportFormDeck[supportIndex%b._frSupportFormDeck.length];wave.push({name:raw.name||'援軍',branch:Array.isArray(raw.branch)?raw.branch.join('、'):(raw.branch||'其他分店'),formId:formId,x:side<0?-55:CW+55,y:Math.min(CH-115,player.y+55),tx:CW/2+side*68,ty:Math.min(CH-120,player.y+45),side:side,hp:maxHp,maxHp:maxHp,attackAt:b.timer+55+i*12,guardAt:b.timer+25,spawnAt:b.timer,departing:false,dead:false});}
  b._frSupporters=wave;burst(CW/2,Math.min(CH-120,player.y+45),'#fef08a',26);frThunderSupportSpeak(wave[0],frThunderSupportLine(FR_THUNDER_SUPPORT_ENTRY_LINES,Math.round(mark*100)+b._frSupportSerial),'#fef08a');
  frBossLater(b,42,function(){if(wave[1]&&!wave[1].dead)frThunderSupportSpeak(wave[1],frThunderSupportLine(FR_THUNDER_SUPPORT_ENTRY_LINES,Math.round(mark*137)+b._frSupportSerial+7),'#bae6fd');});
}
function frThunderSupportTarget(b,s){const left=frThunderHand(b,'leftHand'),right=frThunderHand(b,'rightHand');if(left&&right)return s.side<0?left:right;return left||right||b;}
function frThunderSupportDamageTarget(b,target,damage){
  if(!target)return;if(target===b){if(b.hp<=b.maxHp*.01)return;b.hp=Math.max(b.maxHp*.01,b.hp-damage);updateBossHp();addText('-'+damage,b.x,b.y-72,'#fef08a',12,-.35);return;}
  target.hp=Math.max(0,target.hp-damage);addText('-'+damage,target.x,target.y-45,'#bae6fd',11,-.35);burst(target.x,target.y,'#7dd3fc',8);if(target.hp<=0){target.dead=true;target.mode='idle';addText(target.kind==='leftHand'?'雷爪擊破':'雷掌擊破',target.x,target.y-62,'#fef08a',14,-.35);burst(target.x,target.y,'#ffffff',22);}
}
function frThunderSupportLegacy(b,s){
  const id=s.formId,color=(FR_FORM_MAP[id]||{}).color||'#fef08a',name=FR_THUNDER_SUPPORT_ULTIMATES[id]||'最後一擊',healing=['healing_mushroom','honey_priest','peach_divine'].indexOf(id)>=0,guard=['onion_guard','garlic_knight'].indexOf(id)>=0,control=['coffee_pilot','cheese_mage'].indexOf(id)>=0;
  if(typeof useSkill1==='function'&&typeof useSkill2==='function'&&typeof FOOD_FORMS!=='undefined'&&FOOD_FORMS[id]){const priorForm=currentForm,priorStamina=stamina,priorSk1=sk1Cd,priorSk2=sk2Cd,level=id==='cocoa_popsicle_wargod'?1:2;currentForm=Object.assign({},FOOD_FORMS[id],{id:id});stamina=Math.max(stamina,200);if(level===1)sk1Cd=0;else sk2Cd=0;try{if(level===1)useSkill1();else useSkill2();}finally{const casted=level===1?sk1Cd>0:sk2Cd>0;currentForm=priorForm;stamina=priorStamina;sk1Cd=priorSk1;sk2Cd=priorSk2;if(typeof updateSkillUI==='function')updateSkillUI();updateHUD();if(casted)return;}}
  addText(name,s.x,s.y-48,color,16,-.45);burst(s.x,s.y,color,30);
  if(healing){const rate=id==='peach_divine'?.6:.3,heal=Math.max(35,Math.round(player.maxHp*rate));player.hp=Math.min(player.maxHp,player.hp+heal);if(charSlots[activeChar])charSlots[activeChar].hp=player.hp;updateHUD();addText('+'+heal,player.x,player.y-45,'#86efac',16,-.5);burst(player.x,player.y,'#86efac',28);return;}
  if(guard){player.shieldActive=true;player.shieldHits=Math.max(player.shieldHits||0,id==='onion_guard'?3:4);player.shieldTimer=Math.max(player.shieldTimer||0,300);eBullets=[];burst(player.x,player.y,'#7dd3fc',28);return;}
  if(control){eBullets=[];frV2Targets().forEach(function(t){t.frozenTimer=Math.max(t.frozenTimer||0,id==='coffee_pilot'?180:90);});burst(CW/2,CH*.45,color,34);return;}
  const target=frThunderSupportTarget(b,s),damage=id==='dragonfruit_emperor'?420:id==='lemon_battery'||id==='salmon_ronin'?330:id==='cocoa_popsicle_wargod'?180:260;frThunderSupportDamageTarget(b,target,damage);
  if(['popcorn','lotus_archer','lobster_general','truffle_thunder','dragonfruit_emperor','salmon_ronin'].indexOf(id)>=0){for(let i=0;i<6;i++){const a=-Math.PI*.8+i*Math.PI*.12;ctx.save();ctx.strokeStyle=color;ctx.lineWidth=4;ctx.globalAlpha=.75;ctx.beginPath();ctx.moveTo(s.x,s.y);ctx.lineTo(s.x+Math.cos(a)*220,s.y+Math.sin(a)*220);ctx.stroke();ctx.restore();}}
}
function frThunderSupportDepart(b,s){if(!s||s.departing)return;s.departing=true;s.dead=true;frThunderSupportLegacy(b,s);frThunderSupportSpeak(s,frThunderSupportLine(FR_THUNDER_SUPPORT_LAST_LINES,b._frSupportSerial*5+Math.round(s.spawnAt)),'#fef08a');s.fadeUntil=b.timer+75;}
function frThunderUpdateSupport(b){
  if(!b||!b._frFinal||b._frThunderPhase!==2||b._frTransition||b._frFinalDeath)return;frThunderSupportInit(b);
  const ratio=b.hp/Math.max(1,b.maxHp);while(b._frSupportNext<FR_THUNDER_SUPPORT_THRESHOLDS.length&&ratio<=FR_THUNDER_SUPPORT_THRESHOLDS[b._frSupportNext])b._frSupportQueue.push(FR_THUNDER_SUPPORT_THRESHOLDS[b._frSupportNext++]);
  b._frSupporters=b._frSupporters.filter(function(s){return !s.dead||b.timer<(s.fadeUntil||0);});const living=b._frSupporters.filter(function(s){return !s.dead;});if(!living.length&&b._frSupportQueue.length)frThunderSupportSpawnWave(b,b._frSupportQueue.shift());
  b._frSupporters.forEach(function(s){if(s.dead)return;s.tx=Math.max(55,Math.min(CW-55,player.x+s.side*64));s.ty=Math.min(CH-90,Math.max(CH*.48,player.y+52));s.x+=(s.tx-s.x)*.09;s.y+=(s.ty-s.y)*.09;if(b.timer>=s.attackAt){s.attackAt=b.timer+38;const target=frThunderSupportTarget(b,s);if(target)b._frSupportShots.push({x:s.x,y:s.y,tx:target.x,ty:target.y,target:target,damage:72,color:(FR_FORM_MAP[s.formId]||{}).color||'#fef08a',life:24});}if(b.timer-s.spawnAt>=900)frThunderSupportDepart(b,s);});
  for(let i=b._frSupportShots.length-1;i>=0;i--){const shot=b._frSupportShots[i],target=shot.target;if(target&&target!==b&&!target.dead){shot.tx=target.x;shot.ty=target.y;}else if(target===b){shot.tx=b.x;shot.ty=b.y;}const dx=shot.tx-shot.x,dy=shot.ty-shot.y;shot.x+=dx*.24;shot.y+=dy*.24;shot.life--;if(Math.hypot(dx,dy)<15||shot.life<=0){frThunderSupportDamageTarget(b,target,shot.damage);b._frSupportShots.splice(i,1);}}
}
function frThunderDrawSupport(b){if(!b||!Array.isArray(b._frSupporters))return;b._frSupportShots.forEach(function(shot){ctx.save();ctx.strokeStyle=shot.color;ctx.lineWidth=3;ctx.globalAlpha=.75;ctx.beginPath();ctx.moveTo(shot.x,shot.y+15);ctx.lineTo(shot.x-(shot.tx-shot.x)*.18,shot.y-(shot.ty-shot.y)*.18);ctx.stroke();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(shot.x,shot.y,5,0,Math.PI*2);ctx.fill();ctx.restore();});b._frSupporters.forEach(function(s){const fading=s.dead,alpha=fading?Math.max(0,(s.fadeUntil-b.timer)/75):1,img=s.formId&&s.formId!=='normal'&&typeof frImg==='function'?frImg(s.formId,'battle'):(typeof heroNormalImg!=='undefined'?heroNormalImg:null),bob=Math.sin((b.timer+s.side*9)*.12)*3;ctx.save();ctx.translate(s.x,s.y+bob);ctx.globalAlpha=alpha;ctx.shadowColor=(FR_FORM_MAP[s.formId]||{}).color||'#fef08a';ctx.shadowBlur=10;if(img&&img.complete&&img.naturalWidth){ctx.imageSmoothingEnabled=false;ctx.drawImage(img,-34,-42,68,68);}else{ctx.fillStyle='#f8fafc';ctx.beginPath();ctx.arc(0,-5,25,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=alpha*.9;ctx.fillStyle='rgba(15,23,42,.82)';ctx.fillRect(-31,28,62,6);ctx.fillStyle='#22c55e';ctx.fillRect(-31,28,62*Math.max(0,s.hp/s.maxHp),6);ctx.font='bold 9px sans-serif';ctx.textAlign='center';ctx.fillStyle='#fff';ctx.fillText(s.name,0,46);ctx.font='8px sans-serif';ctx.fillStyle='#bae6fd';ctx.fillText(s.branch,0,57);ctx.restore();});}
...
function frThunderUpdateHands(b){
  ...
  b._frHands.forEach(function(hand){if(hand.dead)return;const home=frThunderHandHome(b,hand);
    ...
    if(hand.mode==='hunt'){
      ...
      const coreX=b._frClapCoreX||b.x,coreY=b._frClapCoreY||b.y+100,fired=b.timer>=(b._frClapFireAt||Infinity),tx=coreX,ty=coreY+(hand.kind==='leftHand'?-72:66)+(fired?10:0);hand.x+=(tx-hand.x)*.22;hand.y+=(ty-hand.y)*.22;if(b.timer>=hand.actionEnd)hand.mode='idle';
    }else if(hand.mode==='slam'){
      if(b.timer>=hand.actionEnd)hand.mode='idle';
    }else if(hand.mode==='cannonLink'){
      const coreX=b._frPalmCoreX||b.x,coreY=b._frPalmCoreY||b.y+88,tx=coreX,ty=coreY+(hand.kind==='leftHand'?-72:66);hand.x+=(tx-hand.x)*.24;hand.y+=(ty-hand.y)*.24;if(b.timer>=hand.actionEnd)hand.mode='idle';
...
const frBossFx=[];
const FR_BOSS_SKILL_ASSET_FILES={
  jellyClones:'jelly-clones.png',crownSlam:'jelly-crown-slam.png',
  creamBaptism:'cream-baptism.png',mechMissile:'heavy-mech-missile.png',pressureCannon:'heavy-mech-pressure-cannon.png',
  coffeePools:'tiramisu-coffee-pools.png',espressoSweep:'tiramisu-espresso-sweep.png',niuCleaver:'niu-cleaver.png',hornQuake:'niu-horn-quake.png',
  woolCloud:'yang-wool-cloud.png',bloodBats:'mala-blood-bats.png',boilingBloodHunt:'mala-boiling-hunt.png',
  iceFlower:'white-ice-flower.png',frostfireCorridor:'white-frostfire-corridor.png',bitterSeedling:'bitter-seedling.png',bitterVineChase:'bitter-vine-chase.png',
  pumpkinRoll:'pumpkin-roll.png',pumpkinChainBurst:'pumpkin-chain-burst.png',karaageOilBomb:'karaage-oil-bomb.png',fryerShockDrop:'karaage-fryer-drop.png',
  eelShadow:'eel-shadow.png',sauceRushTrail:'eel-sauce-rush.png',riceTornado:'fried-rice-tornado.png',wokFlipBurst:'fried-rice-wok-burst.png',
  durianIceLance:'durian-ice-lance.png',frostThornWheel:'durian-frost-wheel.png',mangoSnowcone:'mango-snowcone.png',
  friesIaido:'fries-iaido.png',swordNet:'fries-sword-net.png',noodleBindGrid:'pasta-bind-grid.png',
  inkEightThrust:'octopus-ink-thrust.png',eightArmCombo:'octopus-eight-arm.png'
};
...
function frBossSkillAssetBoundsFor(img,frames){
  ...
  try{
    ...
    let left=frameW,top=img.naturalHeight,right=0,bottom=0;
    for(let frame=0;frame<count;frame++){
      const offset=frame*frameW;
      for(let y=0;y<img.naturalHeight;y++)for(let x=0;x<frameW;x++){
        if(pixels[((y*canvas.width)+offset+x)*4+3]<=8)continue;
        if(x<left)left=x;if(x+1>right)right=x+1;if(y<top)top=y;if(y+1>bottom)bottom=y+1;
      }
    }
    const pad=2,x=Math.max(0,left-pad),y=Math.max(0,top-pad),bounds=right>left&&bottom>top?{x:x,y:y,w:Math.min(frameW,right+pad)-x,h:Math.min(img.naturalHeight,bottom+pad)-y}:fallback;
    frBossSkillAssetBounds.set(img,bounds);return bounds;
  }catch(error){frBossSkillAssetBounds.set(img,fallback);return fallback;}
}
const FR_BOSS_SKILL_ASSET_FX={
  ...
  cleaverLanes:{asset:'niuCleaver',motion:'projectile',duration:105,drawW:92,drawH:154,spin:Math.PI*5,angleOffset:-Math.PI/2},hornQuake:{asset:'hornQuake',motion:'impact',follow:1,duration:112,drawW:190,drawH:105},
  woolCloudSiege:{asset:'woolCloud',motion:'orbit',follow:1,duration:150,drawW:185,drawH:123},
  bloodBatHunt:{asset:'bloodBats',motion:'projectile',duration:125,drawW:36,drawH:24,angleOffset:-Math.PI/2},boilingBloodHunt:{asset:'boilingBloodHunt',motion:'trail',follow:1,duration:180,drawW:245,drawH:123,alpha:.72},
  ...
  snowcone:{asset:'mangoSnowcone',motion:'fall',target:1,duration:112,drawW:132,drawH:132},
  ...
  noodleBindGrid:{asset:'noodleBindGrid',motion:'pulse',target:2,duration:175,drawW:285,drawH:285,alpha:.54},
  inkEightThrust:{asset:'inkEightThrust',motion:'sweep',follow:1,duration:115,drawW:215,drawH:108},eightArmCombo:{asset:'eightArmCombo',motion:'spin',target:1,duration:140,drawW:265,drawH:265,alpha:.76}
};
function frBossSkillAssetFx(b,pattern){
  const base=FR_BOSS_SKILL_ASSET_FX[pattern];
  if(!base||!b)return;
  const cue=Math.max(30,Math.round((frBossTempo(stage).warningMin||42)*.82)),f=Object.assign({},base,{color:b.color||'#fef08a',follow:!!base.follow,start:b.timer+cue});
  if(base.target===1){f.x=player.x;f.y=player.y;f.follow=false;}
  else if(base.target===2){f.x=CW/2;f.y=CH*.54;f.follow=false;}
  else if(!base.follow){
    f.x=b.x;f.y=b.y;f.follow=false;f.toX=player.x-b.x;f.toY=player.y-b.y;f.rotate=Math.atan2(f.toY,f.toX);
  }
  if(base.follow&&(base.motion==='sweep'||base.motion==='trail'))f.rotate=frBossAim(b,true);
  f.rotate=(Number(f.rotate)||0)+(Number(base.angleOffset)||0);
  const lineCue=!base.target&&(base.motion==='projectile'||base.motion==='sweep'||base.motion==='trail');
  if(lineCue){const angle=Math.atan2(player.y-b.y,player.x-b.x);b._frWarnings.push({kind:'line',start:b.timer,end:b.timer+cue,color:b.color,x:b.x,y:b.y,angle:angle});}
  else{const radius=Math.max(42,Math.min(150,Math.max(Number(base.drawW)||80,Number(base.drawH)||80)*.52));b._frWarnings.push({kind:'circle',start:b.timer,end:b.timer+cue,color:b.color,x:f.x==null?b.x:f.x,y:f.y==null?b.y:f.y,r:radius});}
  frBossFxPush(b,'asset',f);
}
...
function frBossFxPush(b,kind,options){
  if(!b)return;
  const quality=window.FR_PERF_QUALITY||'high',cap=FR_BOSS_FX_MOBILE?(quality==='low'?8:quality==='medium'?10:12):36;
...
function frBossDrawFx(b){
  for(let i=frBossFx.length-1;i>=0;i--){
    ...
    ctx.save();ctx.translate(x,y);ctx.globalCompositeOperation=FR_BOSS_FX_MOBILE?'source-over':'lighter';ctx.strokeStyle=f.color;ctx.fillStyle=f.color;ctx.shadowColor=f.color;ctx.shadowBlur=FR_BOSS_FX_MOBILE?0:13;
    if(f.kind==='asset'){
      const img=frBossSkillAssetImage(f.asset);
      if(img&&img.complete&&img.naturalWidth){
        const ease=1-Math.pow(1-p,3),fromX=Number(f.fromX)||0,fromY=Number(f.fromY)||0,toX=Number(f.toX)||0,toY=Number(f.toY)||0;
        const opacity=p<.14?p/.14:p>.82?(1-p)/.18:1,baseScale=Number(f.scale)||1,pulse=f.pulse?1+Math.sin(p*Math.PI)*Number(f.pulse):1,motion=f.motion||'';
        let motionX=0,motionY=0,scaleX=1,scaleY=1,motionRotate=0;
        if(motion==='bounce'){motionY=-Math.abs(Math.sin(p*Math.PI*2))*58;scaleX=scaleY=.94+.06*ease;}
        else if(motion==='fall'){motionY=-190*Math.pow(1-p,2);if(p>.68)scaleX=scaleY=1+Math.sin(Math.min(1,(p-.68)/.2)*Math.PI)*.08;}
        else if(motion==='rise'){motionY=105*(1-ease);scaleX=scaleY=.55+.45*ease;}
        else if(motion==='sweep'){motionRotate=(-.42+p*.84);scaleX=scaleY=.82+.18*ease;}
        else if(motion==='projectile'){motionRotate=Math.sin(p*Math.PI*5)*.08;scaleX=scaleY=1.02+.05*Math.sin(p*Math.PI);}
        else if(motion==='spin'){motionRotate=p*Math.PI*1.35;scaleX=scaleY=.72+.28*ease;}
        else if(motion==='orbit'){motionX=Math.cos(p*Math.PI*4)*48;motionY=Math.sin(p*Math.PI*4)*24;motionRotate=p*Math.PI*.8;scaleX=scaleY=.86+.14*ease;}
        else if(motion==='pulse'){scaleX=scaleY=.82+.18*ease+.06*Math.sin(p*Math.PI*6);}
        else if(motion==='slide'){motionX=(-CW*.65)*(1-ease);scaleX=scaleY=.88+.12*ease;}
        else if(motion==='trail'){motionX=-42*(1-p);scaleX=scaleY=.78+.22*ease;}
        else if(motion==='impact'){scaleX=scaleY=.62+.38*ease;if(p>.55)scaleX=scaleY=1+Math.sin(Math.min(1,(p-.55)/.28)*Math.PI)*.12;}
        ...
        const frames=FR_BOSS_SKILL_ASSET_FRAMES[f.asset]||1,frame=frames>1?Math.floor(age/6)%frames:0,sourceW=img.naturalWidth/frames,bounds=frBossSkillAssetBoundsFor(img,frames)||{x:0,y:0,w:sourceW,h:img.naturalHeight};
        const fit=f.fitMode==='contain'?Math.min(width/bounds.w,height/bounds.h):Math.max(width/bounds.w,height/bounds.h),renderW=bounds.w*fit,renderH=bounds.h*fit;
        ctx.drawImage(img,frame*sourceW+bounds.x,bounds.y,bounds.w,bounds.h,-renderW*anchorX,-renderH*anchorY,renderW,renderH);
        if(frames>1){
...
function frBossCast(b,pattern,isSkill){
  ...
  if(isSkill)frBossSkillAssetFx(b,pattern);
  if(pattern==='cocoaFan'){frBossWarning(b,'cone',w,{angle:frBossAim(b),spread:.58});frBossLater(b,w,function(){frBossFan(b,5,4.6,d,.18,c);});}
  ...
  else if(pattern==='jellyClones'){const cue=Math.max(30,Math.round(w*.82));b._frWarnings.push({kind:'circle',start:b.timer,end:b.timer+cue,color:c,x:b.x,y:b.y+28,r:92});frBossFxPush(b,'asset',{asset:'jellyClones',motion:'bounce',x:b.x,y:b.y+28,follow:false,start:b.timer+cue,duration:72,drawW:176,drawH:117,toY:96,pulse:.05});frBossLater(b,w,function(){frBossSummonMinions(b,2,1);});for(let s=-1;s<=1;s++)frBossLater(b,w+s*8,function(){frBossFan({x:b.x+s*70,y:b.y,color:c},3,4,d,.22,c);});}
  ...
  else if(pattern==='cleaverLanes'){const a=frBossAim(b,true),reach=Math.max(CW,CH)*1.45;frBossHazard({kind:'line',x:b.x,y:b.y,x2:b.x+Math.cos(a)*reach,y2:b.y+Math.sin(a)*reach,w:68,color:c,damage:d,delay:w,minDelay:w,duration:20});}
  ...
  else if(pattern==='crownSlam'){for(let q=0;q<3;q++)frBossLater(b,q*48,function(){const x=player.x+(player.vx||0)*12,y=player.y+(player.vy||0)*12,cue=58,r=68+q*7;b._frWarnings.push({kind:'circle',start:b.timer,end:b.timer+cue,color:c,x:x,y:y,r:r});frBossFxPush(b,'asset',{asset:'crownSlam',motion:'fall',x:x,y:y,follow:false,start:b.timer+cue,duration:68,drawW:190+q*14,drawH:199+q*15,pulse:.04});frBossHazard({kind:'circle',x:x,y:y,r:r,color:c,damage:d,delay:cue,duration:18});frBossLater(b,cue,function(){if(!b._defeated){b.x=Math.max(60,Math.min(CW-60,x));b.y=Math.max(frBossSafeCenterY(b),Math.min(CH*.52,y));}});});}
  ...
  else if(pattern==='bloodBatHunt'){for(let q=0;q<6;q++)frBossLater(b,q*16,function(){const a=frBossAim(b,true)+(q-2.5)*.12;frBossShot(b,a,4.8,d,'#fb7185',12,{curve:q%2?.012:-.012,burn:true,status:{kind:'burn',duration:2400}});});}
  ...
  else if(pattern==='thousandFries'){for(let q=0;q<4;q++)frBossLater(b,q*38,function(){const gap=Math.floor(Math.random()*6),active=[];for(let i=0;i<7;i++)if(i!==gap&&i!==gap+1)active.push(i);const full=frBossShuffle(active).slice(0,2);active.forEach(function(i){const reachesBottom=full.indexOf(i)>=0;frBossHazard({kind:'rect',x:(i+.5)*CW/7,y:reachesBottom?CH/2:CH*.48,w:20,h:reachesBottom?CH:CH*.82,color:'#fbbf24',damage:d,delay:90,minDelay:90,duration:22});});});}
  ...
  else if(pattern==='deepSeaSlam'){for(let q=0;q<4;q++)frBossLater(b,q*32,function(){const left=q%2===0,reachesBottom=q>=2;frBossHazard({kind:'rect',x:left?CW*.18:CW*.82,y:reachesBottom?CH/2:CH*.52,w:CW*.36,h:reachesBottom?CH:CH*.78,color:'#4338ca',damage:d,status:{kind:'attackDown',duration:1800},delay:46,duration:22});});}
  else if(pattern==='missileLock'){for(let q=0;q<4;q++)frBossLater(b,q*28,function(){const x=player.x+(player.vx||0)*14,y=player.y+(player.vy||0)*14,a=Math.atan2(y-b.y,x-b.x),cue=48;b._frWarnings.push({kind:'line',start:b.timer,end:b.timer+cue,color:'#22d3ee',x:b.x,y:b.y,angle:a});b._frWarnings.push({kind:'circle',start:b.timer,end:b.timer+cue,color:'#22d3ee',x:x,y:y,r:48+q*3});frBossFxPush(b,'asset',{asset:'mechMissile',x:b.x,y:b.y,follow:false,start:b.timer+cue,duration:62,drawW:112,drawH:75,toX:x-b.x,toY:y-b.y,rotate:a+Math.PI,anchorX:.64});frBossHazard({kind:'circle',x:x,y:y,r:48+q*3,color:'#22d3ee',damage:d,delay:cue,duration:17});});}
  else if(pattern==='railClamp'){const maxWidth=Math.max(72,(CW-frBossSafeCorridor()-player.radius*2)/2);for(let q=0;q<3;q++)frBossLater(b,q*44,function(){const width=Math.min(maxWidth,CW*(.20+q*.05)),reachesBottom=q>=1,y=reachesBottom?CH/2:CH*.52,h=reachesBottom?CH:CH*.78;frBossHazard({kind:'rect',x:width/2,y:y,w:width,h:h,color:'#06b6d4',damage:d,status:{kind:'slow',duration:1800},delay:90,minDelay:90,duration:18});frBossHazard({kind:'rect',x:CW-width/2,y:y,w:width,h:h,color:'#06b6d4',damage:d,status:{kind:'slow',duration:1800},delay:90,minDelay:90,duration:18});});frBossLater(b,150,function(){frBossHazard({kind:'line',x:CW/2,y:100,x2:CW/2,y2:CH,w:30,color:'#67e8f9',damage:d,delay:90,minDelay:90,duration:16});});}
  ...
  else if(pattern==='creamBaptism'){const cue=54;b._frWarnings.push({kind:'rect',start:b.timer,end:b.timer+cue,color:c,x:CW*.2,y:CH/2,w:CW*.38,h:CH});b._frWarnings.push({kind:'rect',start:b.timer,end:b.timer+cue,color:c,x:CW*.8,y:CH/2,w:CW*.38,h:CH});frBossFxPush(b,'asset',{asset:'creamBaptism',x:CW*.2,y:CH*.42,follow:false,start:b.timer+cue,duration:174,drawW:CW*.38,drawH:CH*.9,fromY:-CH*.3,toY:0,alpha:.72,fitMode:'contain'});frBossFxPush(b,'asset',{asset:'creamBaptism',x:CW*.8,y:CH*.42,follow:false,start:b.timer+cue,duration:174,drawW:CW*.38,drawH:CH*.9,fromY:-CH*.3,toY:0,alpha:.72,fitMode:'contain'});const maxWidth=Math.max(72,(CW-frBossSafeCorridor()-player.radius*2)/2);for(let q=0;q<4;q++){const width=Math.min(maxWidth,54+q*24),delay=90+q*24;frBossHazard({kind:'rect',x:width/2,y:CH/2,w:width,h:CH,color:c,damage:d,status:{kind:'attackDown',duration:3000},delay:delay,minDelay:90,duration:16});frBossHazard({kind:'rect',x:CW-width/2,y:CH/2,w:width,h:CH,color:c,damage:d,status:{kind:'attackDown',duration:3000},delay:delay,minDelay:90,duration:16});}}
  ...
  else if(pattern==='pressureCannon'){for(let q=0;q<3;q++)frBossLater(b,q*44,function(){const a=frBossAim(b,true),cue=Math.max(30,48-q*7),reach=Math.max(CW,CH)*1.55,side=68;b._frWarnings.push({kind:'line',start:b.timer,end:b.timer+cue,color:'#67e8f9',x:b.x,y:b.y,angle:a});frBossFxPush(b,'asset',{asset:'pressureCannon',follow:true,start:b.timer+cue,duration:64-q*5,drawW:170,drawH:112,fromX:Math.cos(a+Math.PI/2)*side,fromY:Math.sin(a+Math.PI/2)*side,rotate:a+Math.PI,anchorX:.5,anchorY:.5,pulse:.035,alpha:.96,fitMode:'contain'});frBossHazard({kind:'line',x:b.x,y:b.y,x2:b.x+Math.cos(a)*reach,y2:b.y+Math.sin(a)*reach,w:42-q*8,color:'#67e8f9',damage:d,status:q===2?{kind:'slow',duration:2400}:null,delay:cue,minDelay:24,duration:16});});}
    ...
    const left=frThunderHand(b,'leftHand'),right=frThunderHand(b,'rightHand');if(!left||!right)return;const coreX=Math.max(82,b.x-112),coreY=b.y+88,end=b.timer+210;b._frPalmCoreX=coreX;b._frPalmCoreY=coreY;b._frPalmStart=b.timer;b._frPalmFireAt=b.timer+60;b._frPalmEnd=end;left.mode='cannonLink';right.mode='cannonLink';left.actionEnd=right.actionEnd=end;b._frBusyUntil=Math.max(b._frBusyUntil,b.timer+220);const emitter={get x(){return (left.x+right.x)/2;},get y(){return (left.y+right.y)/2;},get dead(){return left.dead||right.dead;}};hazards.push(new FrThunderTrackingLaser(b,emitter,Math.round(d*.82)));addText('掌心雷砲',coreX,coreY-72,'#fde047',14,-.35);
  }
...
function frThunderApplyPhase(b,phase,refill){
  ...
  if(phase===2){b._frSupportReady=false;b._frSupportNext=0;b._frSupportQueue=[];b._frSupporters=[];b._frSupportShots=[];}
...
function frBossUpdateCustom(b){
  ...
  if(b._frAttackCd<=0){
    ...
    if(useSkill){
      ...
      if(b._frFinal&&b._frThunderPhase===2){const left=frThunderHand(b,'leftHand'),right=frThunderHand(b,'rightHand');if(!left&&['thunderClawSlam','thunderClawHunt','thunderPalmBarrier'].indexOf(skill)>=0)skill='ninefoldJudgment';if(!right&&['thunderSealHeal','thunderInfusion'].indexOf(skill)>=0)skill='thunderDomain';if((!left||!right)&&['palmThunderCannon','thunderPalmClap'].indexOf(skill)>=0)skill='finalThunderstorm';}
...
function frBossDrawCustom(b){
  ...
  frThunderDrawPowerAura(b);
  if(b._frFinal&&b._frThunderPhase===2){const support=frThunderHand(b,'rightHand');if(support&&(support.mode==='heal'||support.mode==='infuse')){const supportImg=frThunderRelicImage('rightHandAnim'),pulse=.5+.5*Math.sin(b.timer*.18),size=162;ctx.save();ctx.translate(support.x,support.y);ctx.globalAlpha=.94;ctx.shadowColor=support.mode==='heal'?'#86efac':'#fde047';ctx.shadowBlur=FR_BOSS_FX_MOBILE?9:18+pulse*7;frThunderDrawHandSprite(supportImg,b.timer,-size/2,-size/2,size,size);ctx.strokeStyle=support.mode==='heal'?'#bbf7d0':'#fef08a';ctx.lineWidth=4;ctx.globalAlpha=.7;for(let ring=0;ring<2;ring++){ctx.beginPath();ctx.arc(0,0,54+ring*15+pulse*6,0,Math.PI*2);ctx.stroke();}ctx.restore();}}
  ...
  if(b._frFinal&&b._frThunderPhase===2&&Array.isArray(b._frHands)){
    ...
    if(clapActive){const coreX=b._frClapCoreX,coreY=b._frClapCoreY,charge=Math.max(0,Math.min(1,(b.timer-(b._frClapStart||b.timer))/90)),fired=b.timer>=(b._frClapFireAt||Infinity),pulse=.5+.5*Math.sin(b.timer*.35);ctx.save();ctx.globalCompositeOperation=FR_BOSS_FX_MOBILE?'source-over':'lighter';ctx.strokeStyle='#fde047';ctx.lineWidth=3+pulse*2;ctx.shadowColor='#facc15';ctx.shadowBlur=FR_BOSS_FX_MOBILE?0:18;[clapLeft,clapRight].forEach(function(hand){ctx.beginPath();ctx.moveTo(hand.x,hand.y);ctx.lineTo(coreX+(Math.random()-.5)*8,coreY+(Math.random()-.5)*8);ctx.stroke();});const radius=12+charge*24+(fired?pulse*7:0);if(FR_BOSS_FX_MOBILE)ctx.fillStyle='#fde047';else{const grad=ctx.createRadialGradient(coreX,coreY,2,coreX,coreY,radius);grad.addColorStop(0,'#ffffff');grad.addColorStop(.35,'#fef08a');grad.addColorStop(1,'rgba(250,204,21,0)');ctx.fillStyle=grad;}ctx.beginPath();ctx.arc(coreX,coreY,radius,0,Math.PI*2);ctx.fill();ctx.restore();}
    const palmLeft=frThunderHand(b,'leftHand'),palmRight=frThunderHand(b,'rightHand'),palmActive=palmLeft&&palmRight&&palmLeft.mode==='cannonLink'&&palmRight.mode==='cannonLink'&&b.timer<(b._frPalmEnd||0);
    if(palmActive){const coreX=b._frPalmCoreX,coreY=b._frPalmCoreY,charge=Math.max(0,Math.min(1,(b.timer-(b._frPalmStart||b.timer))/60)),fired=b.timer>=(b._frPalmFireAt||Infinity),pulse=.5+.5*Math.sin(b.timer*.42);ctx.save();ctx.globalCompositeOperation=FR_BOSS_FX_MOBILE?'source-over':'lighter';ctx.strokeStyle=fired?'#ffffff':'#fde047';ctx.lineWidth=3+pulse*3;ctx.shadowColor='#facc15';ctx.shadowBlur=FR_BOSS_FX_MOBILE?0:20;[palmLeft,palmRight].forEach(function(hand){for(let arc=0;arc<2;arc++){ctx.beginPath();ctx.moveTo(hand.x,hand.y);ctx.lineTo(coreX+(Math.random()-.5)*10,coreY+(Math.random()-.5)*10);ctx.stroke();}});const radius=10+charge*22+pulse*5;if(FR_BOSS_FX_MOBILE)ctx.fillStyle=fired?'#ffffff':'#facc15';else{const grad=ctx.createRadialGradient(coreX,coreY,2,coreX,coreY,radius);grad.addColorStop(0,'#ffffff');grad.addColorStop(.3,'#fef08a');grad.addColorStop(.68,'#facc15');grad.addColorStop(1,'rgba(250,204,21,0)');ctx.fillStyle=grad;}ctx.beginPath();ctx.arc(coreX,coreY,radius,0,Math.PI*2);ctx.fill();ctx.restore();}
    b._frHands.forEach(function(hand){if(hand.dead)return;const img=frThunderRelicImage(hand.kind+'Anim'),pulse=.5+.5*Math.sin(b.timer*.16+(hand.kind==='leftHand'?0:Math.PI)),barrier=hand.mode==='barrier',channel=hand.mode==='heal'||hand.mode==='infuse',clap=hand.mode==='clap',palm=hand.mode==='cannonLink',linked=clap||palm;if(!channel){ctx.save();const shake=barrier?Math.sin(b.timer*.8)*2:linked?Math.sin(b.timer*.7)*1.5:0;ctx.translate(hand.x+shake,hand.y);if(hand.kind==='leftHand'&&hand.mode==='slam')ctx.rotate(Math.PI/2);if(barrier)ctx.rotate(Math.atan2(player.y-b.y,player.x-b.x)+Math.PI/2);ctx.globalAlpha=.94;ctx.shadowColor='#fde047';ctx.shadowBlur=FR_BOSS_FX_MOBILE?8:14+pulse*6;const size=barrier?162:linked?(hand.kind==='leftHand'?156:136):108;if(!frThunderDrawHandSprite(img,b.timer,-size/2,-size/2,size,size)){ctx.fillStyle=hand.kind==='leftHand'?'#2563eb':'#facc15';ctx.beginPath();ctx.arc(0,0,size*.38,0,Math.PI*2);ctx.fill();}if(barrier){ctx.strokeStyle='#fef08a';ctx.lineWidth=5;ctx.globalAlpha=.55+pulse*.25;ctx.beginPath();ctx.arc(0,0,72+pulse*5,0,Math.PI*2);ctx.stroke();}ctx.restore();}const ratio=Math.max(0,hand.hp/hand.maxHp),barW=76,barY=hand.y-(barrier?94:channel?92:linked?94:65);ctx.fillStyle='rgba(15,23,42,.82)';ctx.fillRect(hand.x-barW/2,barY,barW,6);ctx.fillStyle=ratio>.5?'#38bdf8':ratio>.25?'#fbbf24':'#ef4444';ctx.fillRect(hand.x-barW/2,barY,barW*ratio,6);ctx.fillStyle='#f8fafc';ctx.font='bold 9px sans-serif';ctx.textAlign='center';ctx.fillText(hand.kind==='leftHand'?'雷爪':'雷掌',hand.x,barY-4);});
  }
}
...
const frThunderSupportBaseHurt=hurtPlayer;
hurtPlayer=function(raw){
  if(boss&&boss._frFinal&&boss._frThunderPhase===2&&!boss._frTransition&&!boss._frFinalDeath&&Array.isArray(boss._frSupporters)){
    const ready=boss._frSupporters.find(function(s){return !s.dead&&boss.timer>=(s.guardAt||0);});
    if(ready){ready.guardAt=Infinity;ready.hp=0;player.invTimer=Math.max(player.invTimer||0,8);burst(player.x,player.y,'#fef08a',10);frThunderSupportSpeak(ready,frThunderSupportLine(FR_THUNDER_SUPPORT_GUARD_LINES,Math.round(boss.timer)+boss._frSupportSerial),'#fef08a');frThunderSupportDepart(boss,ready);return;}
  }
  return frThunderSupportBaseHurt.apply(this,arguments);
};
