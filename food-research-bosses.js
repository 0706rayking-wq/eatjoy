...
function frBossTempo(stageNum){
  ...
  if(s>=12)return {normalCd:36,skillCd:54,normalBusy:32,skillBusy:44,busyRate:.94,eventScale:.48,warningMin:22,warningMax:29,moveRetarget:42,moveRate:.035};
  return {normalCd:48,skillCd:66,normalBusy:38,skillBusy:52,busyRate:.88,eventScale:.65,warningMin:30,warningMax:39,moveRetarget:58,moveRate:.029};
}
...
function frBossMaybeSummon(b){
  if(!b._frEntered||b._frTransition||b._defeated)return;
...
class FrBossHazard{
  constructor(options){
    ...
    if(this.delay>0){
      ...
      this.delay=Math.min(tempo.warningMax||Infinity,Math.max(floor,Math.round(this.delay*tempo.eventScale)));
    }
  }
  update(){
    this.age++;
...
function frThunderChaserEnabled(){const s=Number(stage);return currentBgIdx===10&&(s===11||s>=22)&&!stageCleared&&!(typeof boss!=='undefined'&&boss&&boss._frTransition);}
...
function frBossCast(b,pattern,isSkill){
  ...
  else if(pattern==='jazzSyncopation'){const first=Math.floor(Math.random()*5);for(let q=0;q<3;q++)frBossLater(b,q*34,function(){const gap=(first+q*2)%5;for(let i=0;i<5;i++)if(i!==gap)frBossHazard({kind:'rect',x:(i+.5)*CW/5,y:CH/2,w:CW/5-9,h:CH,color:'#92400e',damage:d,status:{kind:'slow',duration:1700},delay:52,duration:16});});}
  ...
  else if(pattern==='thousandFries'){for(let q=0;q<4;q++)frBossLater(b,q*27,function(){const gap=(q*2+Math.floor(Math.random()*2))%7;for(let i=0;i<7;i++)if(i!==gap)frBossHazard({kind:'rect',x:(i+.5)*CW/7,y:CH*.48,w:24,h:CH*.82,color:'#fbbf24',damage:d,delay:46,duration:22});});}
  ...
  else if(pattern==='tomatoCoffin'){frBossWallWithGap(b,-1,'#dc2626',d);frBossWallWithGap(b,1,'#dc2626',d);frBossLater(b,55,function(){const safe=Math.floor(Math.random()*9);for(let iy=0;iy<3;iy++)for(let ix=0;ix<3;ix++)if(iy*3+ix!==safe)frBossHazard({kind:'rect',x:(ix+.5)*CW/3,y:120+(iy+.5)*(CH-170)/3,w:CW/3-14,h:(CH-170)/3-14,color:'#ef4444',damage:d,status:{kind:'slow',duration:2200},delay:50,duration:20});});}
  ...
  else if(pattern==='railClamp'){for(let q=0;q<3;q++)frBossLater(b,q*35,function(){const width=CW*(.22+q*.08);frBossHazard({kind:'rect',x:width/2,y:CH*.52,w:width,h:CH*.78,color:'#06b6d4',damage:d,status:{kind:'slow',duration:1800},delay:48,duration:18});frBossHazard({kind:'rect',x:CW-width/2,y:CH*.52,w:width,h:CH*.78,color:'#06b6d4',damage:d,status:{kind:'slow',duration:1800},delay:48,duration:18});});frBossLater(b,112,function(){frBossHazard({kind:'line',x:CW/2,y:100,x2:CW/2,y2:CH,w:38,color:'#67e8f9',damage:d,delay:44,duration:16});});}
...
function frThunderStartTransition(b,nextPhase){
  ...
  eBullets=[];hazards=[];enemies=enemies.filter(function(e){return !e||!e._frBossSummoned;});
...
function frBossUpdateCustom(b){
  ...
  frBossMaybeSummon(b);
  const expertStage=true,masterStage=Number(stage)>=12,warSpeed=typeof frBossWarSpeedScale==='function'?frBossWarSpeedScale(stage):1;
  ...
  if(b._frAttackCd<=0){
    ...
    if(useSkill){
      if(!b._frSkillBag.length)b._frSkillBag=frBossShuffle(frBossHighStageSkillPool(b));const skill=b._frSkillBag.shift();
