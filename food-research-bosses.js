...
const FR_BOSS_SKILL_ASSET_FILES={
  ...
  durianIceLance:'durian-ice-lance.png',frostThornWheel:'durian-frost-spike.png',mangoSnowcone:'mango-snowcone.png',
...
function frBossSkillAssetImage(key){
  ...
  if(!frBossSkillAssetImages[key]){const img=new Image();img.decoding='async';img.onload=function(){const scan=function(){frBossSkillAssetBoundsFor(img,FR_BOSS_SKILL_ASSET_FRAMES[key]||1);};if(typeof requestIdleCallback==='function')requestIdleCallback(scan,{timeout:800});else setTimeout(scan,0);};img.src='assets/food-research/boss-skills/'+FR_BOSS_SKILL_ASSET_FILES[key]+'?v=4';frBossSkillAssetImages[key]=img;}
  return frBossSkillAssetImages[key];
}
const FR_BOSS_SKILL_ASSET_FRAMES=Object.assign(Object.fromEntries(Object.keys(FR_BOSS_SKILL_ASSET_FILES).map(function(key){return [key,4];})),{creamBaptism:1,pressureCannon:1,frostThornWheel:1});
...
const FR_BOSS_SKILL_ASSET_FX={
  ...
  durianIceLances:{asset:'durianIceLance',motion:'projectile',duration:115,drawW:155,drawH:103},frostThornWheel:{asset:'frostThornWheel',motion:'fall',target:1,duration:104,drawW:72,drawH:150},
...
function frBossCast(b,pattern,isSkill){
  ...
  else if(pattern==='frostThornWheel'){const center=Math.max(58,Math.min(CW-58,player.x));for(let q=0;q<5;q++){const x=Math.max(28,Math.min(CW-28,center+(q-2)*58));frBossHazard({kind:'line',x:x,y:-70,x2:x,y2:CH+70,w:30,color:'#bae6fd',damage:d,status:{kind:'freeze',duration:2100},delay:50+q*9,duration:14});}}
  ...
  else if(pattern==='pressureCannon'){for(let q=0;q<3;q++)frBossLater(b,q*44,function(){const a=frBossAim(b,true),cue=Math.max(30,48-q*7),reach=Math.max(CW,CH)*1.55;b._frWarnings.push({kind:'line',start:b.timer,end:b.timer+cue,color:'#67e8f9',x:b.x,y:b.y,angle:a});frBossFxPush(b,'asset',{asset:'pressureCannon',x:b.x,y:b.y,follow:false,start:b.timer+cue,duration:64-q*5,drawW:Math.min(440,reach*.72),drawH:92-q*10,rotate:a+Math.PI,anchorX:.92,pulse:.04,alpha:.88});frBossHazard({kind:'line',x:b.x,y:b.y,x2:b.x+Math.cos(a)*reach,y2:b.y+Math.sin(a)*reach,w:42-q*8,color:'#67e8f9',damage:d,status:q===2?{kind:'slow',duration:2400}:null,delay:48-q*7,minDelay:24,duration:16});});}
...
function frBossDrawCustom(b){
  ...
  if(b._frFinal&&b._frThunderPhase===2&&Array.isArray(b._frHands)){
    ...
    if(palmActive){const coreX=b._frPalmCoreX,coreY=b._frPalmCoreY,charge=Math.max(0,Math.min(1,(b.timer-(b._frPalmStart||b.timer))/60)),fired=b.timer>=(b._frPalmFireAt||Infinity),pulse=.5+.5*Math.sin(b.timer*.42);ctx.save();ctx.globalCompositeOperation='lighter';ctx.strokeStyle=fired?'#ffffff':'#fde047';ctx.lineWidth=3+pulse*3;ctx.shadowColor='#facc15';ctx.shadowBlur=FR_BOSS_FX_MOBILE?8:20;[palmLeft,palmRight].forEach(function(hand){for(let arc=0;arc<2;arc++){ctx.beginPath();ctx.moveTo(hand.x,hand.y);ctx.lineTo(coreX+(Math.random()-.5)*10,coreY+(Math.random()-.5)*10);ctx.stroke();}});const radius=10+charge*22+pulse*5;const grad=ctx.createRadialGradient(coreX,coreY,2,coreX,coreY,radius);grad.addColorStop(0,'#ffffff');grad.addColorStop(.3,'#fef08a');grad.addColorStop(.68,'#facc15');grad.addColorStop(1,'rgba(250,204,21,0)');ctx.fillStyle=grad;ctx.beginPath();ctx.arc(coreX,coreY,radius,0,Math.PI*2);ctx.fill();ctx.restore();}
