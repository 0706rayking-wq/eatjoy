...
function explanationDepartmentValues(record, companyValue, environment = process.env) {
  ...
  if (/外場|行政|洗滌|洗碗/.test(department)) {
    const configuredName = /洗滌|洗碗|行政/.test(department)
      ? 'NUEIP_ADMIN_WASH_DEPARTMENT_VALUES'
      : 'NUEIP_FRONT_WASH_DEPARTMENT_VALUES';
    const defaultDepartment = department.includes('行政')
      ? `${companyValue}_103018`
      : /洗滌|洗碗/.test(department)
        ? `${companyValue}_103017`
        : `${companyValue}_103015`;
    const configured = String(environment[configuredName] || defaultDepartment)
      .split(',')
...
function recordKey(record) {
  return [record?.employeeNumber, record?.date, record?.department]
    .map((value) => String(value || '').trim())
    .join('|');
}

function retryAttempts(environment = process.env) {
  const configured = Number(environment.NUEIP_EXPLANATION_RETRY_ATTEMPTS || 2);
  if (!Number.isFinite(configured)) return 2;
  return Math.max(0, Math.min(2, Math.floor(configured)));
}

function retryDelay(attempt) {
  return Math.min(12000, 2500 * Math.max(1, attempt));
}
  ...
  try {
    ...
    for (const record of records) {
      try {
        ...
        const userSn = await findScheduleUser(page, record);
        const { options } = await openMultiShiftDialog(page, record, userSn);
        const plan = planSchedule([record], options)[0];
        plans.push({ ...plan, department });
      } catch (error) {
        plans.push({ ...record, status: 'manual', reason: String(error.message || error).slice(0, 180) });
      } finally {
        await page.evaluate(() => {
          const dialogs = [...document.querySelectorAll('[role="dialog"]')];
          const dialog = dialogs.reverse().find((item) => getComputedStyle(item).display !== 'none');
          const close = dialog?.querySelector('#close, .ui-dialog-titlebar-close, button.cancel, button[data-dismiss="modal"]');
          close?.click();
        }).catch(() => {});
...
function scheduleDepartment(record) {
  ...
  if (/洗滌|洗碗/.test(value)) return value.includes('南港') ? value : '南港洗滌';
  if (value.includes('行政')) return value.includes('南港') ? value : '饗麻饗辣南港三井Lalaport';
  throw new Error(`無法判斷${record?.name || '員工'}的排班部門`);
}

function scheduleDepartmentCandidates(record) {
  const value = String(record?.department || '').trim();
  if (value.includes('外場')) return [value, '南港三井Lalaport外場', '南港外場'].filter(Boolean);
  if (value.includes('內場')) return [value, '南港三井Lalaport內場', '南港內場'].filter(Boolean);
  if (/洗滌|洗碗/.test(value)) return [value, '南港三井Lalaport洗滌', '南港洗滌', '洗滌', '洗碗'].filter(Boolean);
  if (value.includes('行政')) return [value, '南港三井Lalaport行政', '饗麻饗辣南港三井Lalaport', '南港行政', '行政'].filter(Boolean);
  throw new Error(`無法判斷${record?.name || '員工'}的排班部門`);
}
...
async function openDepartmentSchedule(page, record) {
  const departmentCandidates = scheduleDepartmentCandidates(record);
  await page.goto('https://cloud.nueip.com/dept_shift_schedule_work', {
    waitUntil: 'networkidle2',
    timeout: 30000
  });
  const department = await page.evaluate(({ departmentCandidates, month }) => {
    const rows = [...document.querySelectorAll('tr')];
    const row = rows.find((candidate) => {
      const text = candidate.textContent || '';
      return departmentCandidates.some((department) => text.includes(department)) && text.includes(month);
    });
    const button = row?.querySelector('[data-th="排班"] .Scheduling');
    if (!button) return '';
    const matched = departmentCandidates.find((department) => (row.textContent || '').includes(department)) || '';
    button.click();
    return matched;
  }, { departmentCandidates, month: record.date.slice(0, 7).replace('-', '.') });
  if (!department) throw new Error(`找不到${departmentCandidates[0]}當月班表`);
...
async function findScheduleUser(page, record) {
  await page.evaluate(() => window.scrollTo(0, 0));
  for (let attempt = 0; attempt < 24; attempt += 1) {
    const userSn = await page.evaluate(({ employeeNumber, name }) => {
      const row = [...document.querySelectorAll('tr.shift-tr')].find((candidate) => {
        const firstCellText = (candidate.querySelector('td')?.textContent || '').replace(/\s/g, '');
        const rowText = (candidate.textContent || '').replace(/\s/g, '');
        return firstCellText.includes(employeeNumber)
          || firstCellText.includes(name)
          || rowText.includes(employeeNumber)
...
async function openMultiShiftDialog(page, record, userSn) {
  const weekday = weekdayForDate(record.date);
  ...
  await page.waitForFunction(({ employeeNumber, name, date }) => {
    const dialogs = [...document.querySelectorAll('[role="dialog"]')];
    const dialog = dialogs.reverse().find((item) => getComputedStyle(item).display !== 'none');
    const text = dialog?.textContent || '';
    const startDate = dialog?.querySelector('input[name="start_date"]')?.value;
    const dateMatches = String(startDate || '').replace(/\D/g, '') === String(date).replace(/\D/g, '');
    return (text.includes(employeeNumber) || text.includes(name)) && dateMatches;
  }, { timeout: 15000 }, record).catch(async () => {
    const diagnostic = await page.evaluate(() => {
      const candidates = [...document.querySelectorAll('[role="dialog"], .ui-dialog, .modal, .modal-dialog')]
        .filter((item) => getComputedStyle(item).display !== 'none');
      const dialog = candidates.reverse().find((item) => /排班|班別|快速排班/.test(item.textContent || ''))
        || candidates[0];
      if (!dialog) return '未找到可見視窗';
      const inputs = [...dialog.querySelectorAll('input, select')].slice(0, 24).map((item) => {
        const name = item.getAttribute('name') || item.id || item.tagName.toLowerCase();
        const value = item.value || item.querySelector?.('option:checked')?.textContent || '';
        return `${name}=${String(value).trim()}`;
      });
      return inputs.join(',').slice(0, 150);
    }).catch(() => '診斷讀取失敗');
    throw new Error(`排班視窗人員或日期未就緒（${diagnostic}）`);
  });
...
async function applySchedulePlan(page, record, plan, userSn, weekday) {
  await page.evaluate(({ record, selectedShifts }) => {
    ...
    const employeeMatches = employee.includes(record.employeeNumber) || employee.includes(record.name);
    const expectedDate = String(record.date).replace(/\D/g, '');
    const dateMatches = String(startDate || '').replace(/\D/g, '') === expectedDate
      && String(endDate || '').replace(/\D/g, '') === expectedDate;
    if (!dialog || !employeeMatches || !dateMatches) {
  ...
  await page.waitForFunction(({ userSn, weekday, expected }) => {
    const row = document.querySelector(`tr.shift-tr[data-usn="${userSn}"]`);
    const cell = row?.querySelectorAll(':scope > td.shift-td')?.[weekday];
    const labels = [...new Set((cell?.textContent || '').match(/[早晚全]班\([^)]*\)/g) || [])];
    return JSON.stringify(labels) === JSON.stringify(expected);
  }, { timeout: 8000 }, { userSn, weekday, expected }).catch(() => {});
  ...
  if (JSON.stringify(labels) !== JSON.stringify(expected)) {
    throw new Error(`儲存後班別驗證不一致（預期：${expected.join('、')}；實際：${labels.join('、') || '空白'}）`);
  }
  return labels;
}
    ...
    for (const record of records) {
      try {
        ...
        for (const selectedDepartment of explanationDepartmentValues(record, companyValue)) {
          // Query one employee at a time. The "all employees" result is
          // paginated at 50 rows in NUEIP, which made employees on later pages
          // look as if their modify button did not exist.
          const queryKey = `${record.date}|${selectedDepartment}|${record.employeeNumber}`;
          if (queryCache.key !== queryKey) {
          ...
          const employeeValue = await page.evaluate(({ selectedDepartment, employeeNumber, name }) => {
            const normalize = (value) => String(value || '').replace(/\s/g, '');
            const expectedNumber = normalize(employeeNumber);
            const expectedName = normalize(name);
            const selects = [...document.querySelectorAll('form select')];
            const employeeSelect = selects.find((select) => [...select.options]
              .some((option) => option.value === `${selectedDepartment}_0`));
            const matchingOption = [...(employeeSelect?.options || [])].find((option) => {
              const label = normalize(option.textContent);
              return (expectedNumber && label.includes(expectedNumber))
                || (expectedName && label.includes(expectedName));
            });
            return matchingOption?.value || `${selectedDepartment}_0`;
          }, { selectedDepartment, employeeNumber: record.employeeNumber, name: record.name });
        ...
        await page.waitForFunction(({ employeeNumber, name, date }) => {
          ...
          const dialog = dialogs.reverse().find((item) => getComputedStyle(item).display !== 'none');
          const text = dialog?.textContent || '';
          // NUEIP displays the selected day with slashes (YYYY/MM/DD), while
          // this workflow uses ISO dates (YYYY-MM-DD). Compare digits so a
          // correctly opened edit dialog is not needlessly left to time out.
          const expectedDate = String(date).replace(/\D/g, '');
          const dialogDate = String(text).replace(/\D/g, '');
          const employeeMatches = text.includes(employeeNumber) || (name && text.includes(name));
          return employeeMatches
            && dialogDate.includes(expectedDate)
            && dialog.querySelector('input[name^="remark["]');
        ...
        const formState = await page.evaluate((expected) => {
          ...
          // The attendance list and the edit dialog do not always classify a
          // middle punch the same way (especially on split/three-shift days).
          // Employee and date were verified above, so the edit dialog is the
          // authoritative source for the values written into its own remarks.
          // Requiring an exact time+type match here caused one harmless display
          // difference to abort the whole department batch.
          const writableRows = rows.filter((row) => row.time && row.remark);
          if (writableRows.length === 0) {
            return { ok: false, reason: '修改視窗找不到可寫入的打卡時間', actualKeys: [] };
          }
          let changed = 0;
          ...
          return { ok: true, changed, total: writableRows.length };
        }, record);
...
async function syncExplanationRecords(value, mode = 'commit') {
  const records = normalizeRecords(value || []);
  if (mode !== 'commit') {
    return { status: 'preview', count: records.length, records };
  }
  if (String(process.env.NUEIP_EXPLANATION_SYNC_ENABLED || 'true').toLowerCase() !== 'true') {
    const error = new Error('NUEIP explanation sync is disabled');
    error.statusCode = 409;
    throw error;
  }
  if (records.length === 0) {
    return { status: 'ok', updated: 0, unchanged: 0, failed: 0, results: [] };
  }
  // Browserbase can occasionally close or lose a session while NUEIP is saving a
  // row.  A retry opens a fresh browser and only repeats the rows that were not
  // saved, so rows already written are never processed again.
  const sourceByKey = new Map(records.map((record) => [recordKey(record), record]));
  const latestByKey = new Map();
  let pending = records;
  const attempts = retryAttempts();
  for (let attempt = 0; pending.length > 0 && attempt <= attempts; attempt += 1) {
    let attemptResults;
    try {
      attemptResults = await syncRecords(pending);
    } catch (error) {
      const message = String(error.message || error).slice(0, 180);
      attemptResults = pending.map((record) => ({
        employeeNumber: record.employeeNumber,
        name: record.name,
        date: record.date,
        department: record.department,
    ...
    for (const result of attemptResults) latestByKey.set(recordKey(result), result);
    pending = attemptResults
      .filter((result) => result.status === 'error')
      .map((result) => sourceByKey.get(recordKey(result)))
      .filter(Boolean);
    if (pending.length > 0 && attempt < attempts) {
      await new Promise((resolve) => setTimeout(resolve, retryDelay(attempt + 1)));
    }
  }
  const results = records.map((record) => latestByKey.get(recordKey(record)) || {
    ...record,
    status: 'error',
    message: '找不到寫入結果'
  });
...
async function handler(request, response) {
  ...
  try {
    ...
    const result = await syncExplanationRecords(records, mode);
...
module.exports.syncExplanationRecords = syncExplanationRecords;
