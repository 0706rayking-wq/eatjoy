...
async function previewSchedules(records) {
  ...
  try {
    ...
    await page.waitForFunction(() => [...document.querySelectorAll('select')].some((select) =>
      [...select.options].some((option) => /\d{1,2}:\d{2}\s*[~～\-]\s*\d{1,2}:\d{2}/.test(option.textContent || ''))
    ), { timeout: 20000 });
    const options = await page.evaluate(() => {
      ...
      const selects = [...document.querySelectorAll('select')]
        .map((select) => ({
          select,
          timedOptions: [...select.options].filter((option) => timePattern.test(option.textContent || ''))
        }))
        .filter((entry) => entry.timedOptions.length > 0)
        .sort((left, right) => right.timedOptions.length - left.timedOptions.length);
      const select = selects[0]?.select;
      if (!select) return [];
