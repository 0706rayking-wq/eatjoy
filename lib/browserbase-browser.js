...
function buildSessionPayload(environment = process.env, options = {}) {
  const config = browserbaseConfig(environment);
  ...
  return {
    ...
    userMetadata: {
      workflow: String(options.workflow || 'nueip-hr-automation')
    }
  };
}
...
async function launchBrowser(environment = process.env, fetchImpl = globalThis.fetch, options = {}) {
  if (!hasBrowserbaseConfig(environment)) return launchLocalBrowser(options);
  ...
  const response = await fetchImpl('https://api.browserbase.com/v1/sessions', {
    ...
    body: JSON.stringify(buildSessionPayload(environment, options))
  });
