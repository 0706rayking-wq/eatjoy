...
function chooseShift(segment, options, isFinal) {
  ...
  // The final segment must never shorten an employee's recorded day. When
  // several shifts start at the same time, deliberately use the one that
  // ends latest rather than the closest match to the handwritten end time.
  const ranked = [...candidates].sort((left, right) =>
    timeMinutes(right.end) - timeMinutes(left.end)
    || left.label.localeCompare(right.label, 'zh-Hant')
  );
  return { status: 'ready', shift: ranked[0] };
}
