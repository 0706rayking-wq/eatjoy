...
(()=>{
 ...
 const aw={drops:[],spirits:[],rifts:[],slashes:[],blasts:[],moveUntil:0,shieldCharges:0,repeat:false};
 ...
 function updateAw(now){
  ...
  for(let i=aw.rifts.length-1;i>=0;i--){const f=aw.rifts[i];if(now>=f.until){aw.rifts.splice(i,1);continue;}targets().forEach(t=>{const d=Math.hypot(t.x-f.x,t.y-f.y)||1;if(d<=f.pullR+(t.r||18)){const pull=(t===boss ? .018 : .055);t.x+=(f.x-t.x)*pull;t.y+=(f.y-t.y)*pull;}if(d<=f.r+(t.r||18)&&now>=f.next)t.takeDamage(15);});if(now>=f.next)f.next=now+1000;}
 ...
 function drawAw(now){ctx.save();
  ...
  aw.rifts.forEach(f=>{const left=Math.max(0,(f.until-now)/2000),pulse=1+.08*Math.sin(now/70+f.phase),g=ctx.createRadialGradient(f.x,f.y,4,f.x,f.y,f.r*pulse);g.addColorStop(0,'rgba(15,23,42,.95)');g.addColorStop(.5,'rgba(88,28,135,.72)');g.addColorStop(1,'rgba(167,139,250,0)');ctx.save();ctx.globalAlpha=.65+.25*left;ctx.fillStyle=g;ctx.beginPath();ctx.arc(f.x,f.y,f.r*pulse,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#c4b5fd';ctx.lineWidth=3;ctx.setLineDash([8,7]);ctx.lineDashOffset=-now/35;ctx.beginPath();ctx.arc(f.x,f.y,f.r*.78,0,Math.PI*2);ctx.stroke();ctx.restore();});
 ...
 const baseBuild=buildStage;buildStage=function(){aw.drops=[];aw.spirits=[];aw.rifts=[];aw.slashes=[];aw.blasts=[];aw.moveUntil=0;aw.shieldCharges=0;return baseBuild.apply(this,arguments);};
