...
(()=>{
 ...
 window.frAwakenMeleeWave=function(def){if(!awake('critDamage')||Math.random()>=.01)return;const now=performance.now();aw.blasts.push({kind:'wave',x:player.x,y:player.y,r:75,vy:-11,until:now+2200});cue('會心劍氣',player.x,player.y,'#e0f2fe');};
 ...
 function updateAw(now){
  ...
  for(let i=aw.blasts.length-1;i>=0;i--){const f=aw.blasts[i];if(f.kind==='wave'){f.y+=f.vy*(window.FR_FRAME_SCALE||1);for(let j=eBullets.length-1;j>=0;j--){const b=eBullets[j],dx=b.x-f.x;if(Math.abs(dx)>f.r+(b.r||5))continue;const arcY=f.y-Math.sqrt(Math.max(0,f.r*f.r-dx*dx));if(Math.abs(b.y-arcY)<=18+(b.r||5))eBullets.splice(j,1);}if(f.y+f.r<0)f.until=0;}if(now>=f.until)aw.blasts.splice(i,1);}
 }
function drawAw(now){if(typeof FR_MOBILE_PERF!=='undefined'&&FR_MOBILE_PERF&&window.FR_PERF_QUALITY==='low'&&(Math.floor(now/16.7)&1))return;ctx.save();
  ...
  aw.blasts.forEach(f=>{const left=(f.until-now)/(f.kind==='wave'?2200:420);ctx.globalAlpha=Math.max(0,Math.min(1,left));ctx.strokeStyle=f.kind==='wave'?'#d1d5db':'#fb7185';ctx.lineWidth=f.kind==='wave'?15:8;ctx.shadowColor=f.kind==='wave'?'#f8fafc':ctx.strokeStyle;ctx.shadowBlur=14;ctx.beginPath();if(f.kind==='wave'){ctx.arc(f.x,f.y,f.r,Math.PI,Math.PI*2);ctx.stroke();ctx.globalAlpha*=.8;ctx.strokeStyle='#6b7280';ctx.lineWidth=4;ctx.beginPath();ctx.arc(f.x,f.y,f.r-9,Math.PI,Math.PI*2);ctx.stroke();}else{ctx.arc(f.x,f.y,50*(1-left*.5),0,Math.PI*2);ctx.stroke();}});ctx.restore();
 }
